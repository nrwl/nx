...
- Make sure unit tests pass (`nx affected --target=test`)
  ...
  - `packages/nx`, `packages/workspace`, `packages/rspack`, `packages/rsbuild`, `packages/angular`, `packages/react`, `packages/plugin`, `packages/js`, `packages/storybook`, `packages/remix`, `packages/next`, `packages/expo`, `packages/react-native`, `packages/eslint` and `packages/oxlint` run on vitest instead: target a file with
