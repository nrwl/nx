...
- Make sure unit tests pass (`nx affected --target=test`)
  ...
  - `packages/nx`, `packages/workspace`, `packages/rspack`, `packages/rsbuild`, `packages/angular`, `packages/react`, `packages/plugin`, `packages/js`, `packages/storybook`, `packages/remix`, `packages/next`, `packages/expo`, `packages/react-native`, `packages/eslint`, `packages/oxlint`, `packages/eslint-plugin`, `packages/devkit`, `packages/web`, `packages/vite`, `packages/webpack`, `packages/vitest`, `packages/rollup`, `packages/esbuild`, `packages/cypress`, `packages/playwright`, `packages/gradle`, `packages/dotnet`, `packages/maven`, `packages/vue`, `packages/nuxt`, `packages/module-federation`, `packages/create-nx-workspace`, `packages/create-nx-plugin`, `packages/node`, `packages/express`, `packages/docker`, `packages/nest` and `packages/detox` run on vitest instead: target a file with
