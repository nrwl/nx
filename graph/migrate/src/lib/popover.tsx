...
export function Popover({ isOpen, onClose, children, position }: PopoverProps) {
  ...
  return (
    <div
      ...
      {...getFloatingProps()}
      className="animate-fadeIn absolute z-50 flex w-64 flex-col gap-1 rounded-md border border-slate-300/[0.25] bg-white shadow-lg transition-opacity duration-200"
    >
      {children}
    </div>
  );
