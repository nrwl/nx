      ...
      switch (event.type) {
         ...
         case 'notifyGraphUpdateGraph':
            ...
            elementsToSendToRender =
               this.projectTraversalGraph.setShownProjects(
         ...
         case 'notifyGraphShowAllProjects':
            elementsToSendToRender =
               this.projectTraversalGraph.showAllProjects();
            break;

         case 'notifyGraphHideAllProjects':
            elementsToSendToRender =
               this.projectTraversalGraph.hideAllProjects();
            break;
         ...
         case 'notifyGraphTracing':
            if (event.start && event.end) {
               if (event.algorithm === 'shortest') {
                  elementsToSendToRender =
                     this.projectTraversalGraph.traceProjects(
                        event.start,
                        event.end
                     );
               } else {
   ...
   handleTaskEvent(event: TaskGraphRenderEvents) {
      ...
      switch (event.type) {
         case 'notifyTaskGraphSetProjects':
            this.taskTraversalGraph.setProjects(
               event.projects,
               event.taskGraphs
            );
            break;
