...
export function ShowOptionsHelp({
   ...
   return (
      helpCommand && (
         <>
            <p className="mb-4">
               Use <code>--help</code> to see all options for this command, and
               set them by{' '}
               <a
                     ...
                     (
                        <div className="w-fit max-w-md">
                           <p className="mb-2">
                              ...
                              configuration for the <code>{targetName}</code>{' '}
                              target in the <code>project.json</code> file for{' '}
                           ...
                           {helpExampleOptions && (
                              <p className="mb-2">
                                 The <code>options</code> are CLI options
                                 prefixed by <code>--</code>, such as{' '}
                                 <code>ls --color=never</code>, where you would
                                 use <code>{'"color": "never"'}</code> to set it
                                 in the target configuration.
                              </p>
                           )}
                           {helpExampleArgs && (
                              <p className="mb-2">
                                 The <code>args</code> are CLI positional
                                 arguments, such as <code>ls somedir</code>,
                                 where you would use{' '}
                                 <code>{'"args": ["somedir"]'}</code> to set it
                                 in the target configuration.
                              </p>
                           )}
                        </div>
               ...
               content={
                  <div className="relative w-full">
                     ...
                     >
                        <pre
                           className={
                              result && !result.success ? 'text-red-500' : ''
                           }
                        >
                           {result?.text}
                        </pre>
