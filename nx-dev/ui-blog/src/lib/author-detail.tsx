...
export default function AuthorDetail({ author }: AuthorDetailProps) {
  return (
    <div className="space-between invisible absolute right-0 left-[65%] z-30 mt-2 flex w-60 translate-x-[-50%] items-center gap-4 rounded-sm bg-zinc-50 p-4 text-sm text-zinc-700 opacity-0 shadow-lg ring-1 ring-zinc-200 transition-all delay-75 duration-300 ease-in-out md:group-hover:visible md:group-hover:opacity-100 dark:bg-zinc-900 dark:text-zinc-400 dark:ring-zinc-800">
      <span>
        <Image
          alt={author.name}
          title={author.name}
