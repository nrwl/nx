...
sidebar:
  label: Configure Vite
filter: 'type:Guides'
---

{% aside type="note" title="Use our generator!" %}
It is recommended that you use the [`@nx/vite:configuration`](/docs/technologies/build-tools/vite/generators#configuration) generator to set up [Vite](https://vite.dev/) for your new or existing projects.
{% /aside %}
...
## What your `vite.config.ts` looks like
...
In that config file, you can configure Vite as you would normally do. For more information, see the [Vite documentation](https://vite.dev/config/).
