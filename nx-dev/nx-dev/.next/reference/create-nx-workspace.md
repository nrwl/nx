...
| ----------- | ----------- | ---------- |
| `--aiAgents` | `claude`, `codex`, `copilot`, `cursor`, `gemini`, `opencode` | List of AI agents to configure. |
| `--allPrompts`, `--a` | boolean | Show all prompts. (Default: `false`) |
| `--analytics` | boolean | Help improve Nx by sharing your usage data. |
...
| `--nxCloud`, `--ci` | `github`, `gitlab`, `azure`, `bitbucket-pipelines`, `circleci`, `skip`, `never`, `yes` | Which CI provider would you like to use? |
...
| `--template` | string | GitHub template repository to use. Available templates: nrwl/empty-template, nrwl/react-template, nrwl/angular-template, nrwl/typescript-template |
...
| `--zoneless` | boolean | Generate an application that does not use 'zone.js'. (Default: `true`) |
