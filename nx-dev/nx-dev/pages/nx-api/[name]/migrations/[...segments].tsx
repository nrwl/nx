import { MigrationViewer } from '@nx/nx-dev/feature-package-schema-viewer';
...
export default function PackageMigration({
  menu,
  pkg,
  schema,
}: {
    ...
    <div id="shell" className="flex h-full flex-col">
      ...
      >
        ...
        <ScrollableContent resetScrollOnNavigation={true}>
          <MigrationViewer pkg={vm.package} schema={vm.schema} />
        </ScrollableContent>
      </main>
    </div>
  );
...
export const getStaticPaths: GetStaticPaths = () => {
  return {
    paths: [
      ...nxPackagesApi.getStaticDocumentPaths().migrations.map((x) => ({
        params: {
...
function getData(
  ...
  return {
    ...
    schema: nxPackagesApi.getSchemaMetadata(
      nxPackagesApi.getPackageFileMetadatas(packageName, 'migrations')[
        '/' + ['nx-api', packageName, 'migrations', ...segments].join('/')
      ]
    ),
