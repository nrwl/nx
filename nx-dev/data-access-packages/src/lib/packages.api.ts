...
export class PackagesApi {
   ...
   getStaticDocumentPaths(): {
      ...
      packages.forEach((p) => {
         experiment.packages.push(
            generateSegments(p.path, this.options.prefix)
         );

         Object.keys(p.documents).map((path) =>
            experiment.documents.push(
               generateSegments(path, this.options.prefix)
            )
         );
         if (p.name === 'devkit') {
            readdirSync('../../docs/generated/devkit').forEach((fileName) => {
               if (fileName.endsWith('.md')) {
                  experiment.documents.push(
                     generateSegments(
                        `packages/devkit/documents/${fileName.replace(
                           '.md',
                           ''
                        )}`,
                        this.options.prefix
                  ...
                  readdirSync(
                     '../../docs/generated/devkit/' + fileName
                  ).forEach((subFileName) => {
                     experiment.documents.push(
                        generateSegments(
         ...
         Object.keys(p.executors).forEach((path) =>
            experiment.executors.push(
               generateSegments(path, this.options.prefix)
            )
         );

         Object.keys(p.generators).forEach((path) =>
            experiment.generators.push(
               generateSegments(path, this.options.prefix)
            )
         );
      });
