    ...
    <>
      ...
      <ul className={cx('mb-6', collapsed ? 'hidden' : '')}>
        {(item.itemList as MenuItem[]).map((subItem, index) => {
          ...
          const isNxCloudDocs = withoutAnchors(router.asPath).startsWith('/nx-cloud');
          ...
          return (
            ...
            >
              <Link href={subItem.path as string}>
                ...
                >
                  {isActiveLink ? (
                    <span className={`${isNxCloudDocs ? 'bg-green-nx-base' : 'bg-blue-nx-base'} absolute -right-2 top-0 h-full w-1 rounded-md sm:-right-4`} />
                  ) : null}
                  <span
