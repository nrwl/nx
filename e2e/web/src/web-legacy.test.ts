...
describe('Web Components Applications (legacy)', () => {
  ...
  it('should remove previous output before building', async () => {
    ...
    runCLI(
      `generate @nx/web:app ${appName} --directory=apps/${appName} --bundler=webpack --no-interactive --compiler swc`,
      {
        env: {
    ...
    runCLI(
      `generate @nx/react:lib ${libName} --directory=libs/${libName} --bundler=rollup --no-interactive --compiler swc --unitTestRunner=jest`,
      {
        env: {
          NX_ADD_PLUGINS: 'false',
        },
