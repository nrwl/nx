...
describe('Web Components Applications', () => {
  ...
  it('should be able to generate a web app', async () => {
    ...
    runCLI(
      `generate @nx/web:app ${appName} --directory=apps/${appName} --bundler=webpack --no-interactive`
    );
  ...
  it('should emit decorator metadata when --compiler=babel and it is enabled in tsconfig', async () => {
    const appName = uniq('app');
    runCLI(
      `generate @nx/web:app ${appName} --directory=apps/${appName} --bundler=webpack --compiler=babel --no-interactive`
    );
  ...
  it('should emit decorator metadata when using --compiler=swc', async () => {
    const appName = uniq('app');
    runCLI(
      `generate @nx/web:app ${appName} --directory=apps/${appName} --bundler=webpack --compiler=swc --no-interactive`
    );
...
describe('CLI - Environment Variables', () => {
  it('should automatically load workspace and per-project environment variables', async () => {
    ...
    runCLI(
      `generate @nx/web:app ${appName} --directory=apps/${appName} --bundler=webpack --no-interactive --compiler=babel`
    );
    ...
    runCLI(
      `generate @nx/web:app ${appName2} --directory=apps/${appName2} --bundler=webpack --no-interactive --compiler=babel`
    );
...
describe('index.html interpolation', () => {
  ...
  test('should interpolate environment variables', async () => {
    ...
    runCLI(
      `generate @nx/web:app ${appName} --bundler=webpack --directory=apps/${appName} --no-interactive`
    );
