...
describe('nx release - custom npm registries', () => {
   ...
   it('should respect registry configuration for each package', async () => {
      ...
      const publishResultNoScopeWithPublishConfigAndProjectConfigAndArg =
         runCLI(
      ...
      const noScopeWithProjectConfig = newPackage(
         'pkg-no-scope-project-config',
         {
            projectConfig: {
   ...
   function newPackage(
      ...
      updateJson<ProjectConfiguration>(
