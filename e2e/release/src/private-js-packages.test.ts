...
describe('nx release - private JS packages', () => {
   ...
   it('should skip private packages and log a warning when private packages are explicitly configured', async () => {
      ...
      const publicPkg1PublishOutput = runCLI(
         `release publish -p ${publicPkg1}`
      );
    ...
    `);
      ...
      const publicPkg2PublishOutput = runCLI(
         `release publish -p ${publicPkg2}`
      );
    ...
    `);
      ...
      const privatePkgPublishOutput = runCLI(
         `release publish -p ${privatePkg}`,
         {
            silenceError: true,
         }
      );
    ...
    `);
      ...
      expect(() =>
         execSync(`npm view @proj/${privatePkg} version`)
