...
describe('nx release - independent projects', () => {
  ...
  it('should allow versioning projects independently', async () => {
    ...
    expect(
      releaseOutput.match(new RegExp(`New version 1\.2\.1 written`, 'g')).length
    ).toEqual(1);

    expect(
      releaseOutput.match(new RegExp(`New version 1\.4\.1 written`, 'g')).length
    ).toEqual(1);

    expect(
      releaseOutput.match(new RegExp(`New version 1\.6\.1 written`, 'g')).length
    ).toEqual(1);
