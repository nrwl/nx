    ...
    `);
      ...
      expect(
         versionOutput2.match(
            /Running release version for project: my-pkg-\d*/g
         ).length
      ).toEqual(3);
      expect(
         versionOutput2.match(
  ...
  `);
    ...
    `);
      ...
      expect(
         releaseOutput.match(
            new RegExp(
               `Generating an entry in CHANGELOG\.md for v1200\.0\.0`,
               'g'
            )
         ).length
      ).toEqual(1);
      ...
      expect(
         releaseOutput2.match(
            new RegExp(`New version 1300\.1\.0 written to`, 'g')
         ).length
      ).toEqual(2);
