...
describe('migrate orchestrator (dark launch)', () => {
  ...
  it('should report the active run on any init, whatever the plan, and continue it on --run-id', () => {
    ...
    expect(samePlan).toContain(
      `${PM_EXEC_PREFIX[getSelectedPackageManager()]} nx migrate --run-migrations --start-fresh`
    );
  ...
  it('should start a new run on --start-fresh and refuse the flag outside the orchestrator', () => {
    ...
    const outside = runCLI(
      'migrate --run-migrations=migrations.json --start-fresh',
      {
        env: {
          NX_MIGRATE_USE_LOCAL: 'true',
          NX_MIGRATE_SKIP_INSTALL: 'true',
    ...
    const fresh = runInit(' --start-fresh');
...
`;
  ...
  describeMaster('master session (dark)', () => {
    ...
    it('should exit 1 with the resume hint when the agent session ends before the run completes', async () => {
      ...
      expect(report.output).toContain(
        'nx migrate --run-migrations --agentic=claude-code --start-fresh'
      );
