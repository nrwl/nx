...
describe('Nx Running Tests', () => {
   ...
   describe('Nx Bail', () => {
      it('should stop executing all tasks when one of the tasks fails', async () => {
         ...
         let withBail = runCLI(
            `run-many --target=error --parallel=1 --nx-bail`,
            {
               silenceError: true,
            }
         )
   ...
   describe('run-one', () => {
      ...
      it('should run targets from package json', () => {
         ...
         expect(
            runCLI(`${target} ${myapp} --configuration production`)
         ).toContain(expectedEnvOutput);
      }, 10000);
  ...
  registerProjectTargets: () => ({
   ...
   describe('run-many', () => {
      ...
      it('should run multiple targets', () => {
         ...
         expect(outputs).toContain(
            'Running targets build, test for 2 projects:'
         );
   ...
   describe('exec', () => {
      ...
      beforeAll(() => {
         ...
         runCLI(
            `generate @nx/js:lib ${pkg} --bundler=none --unitTestRunner=none`
         );
         runCLI(
      ...
      it('should run adhoc tasks in topological order', () => {
         ...
         output = runCLI(`exec -- echo '$NX_PROJECT_NAME'`).replace(
            /\s+/g,
            ' '
         );
         expect(output).toContain(pkg);
