...
module.exports = {
  ...
  plugins: [
   ...
   it('should be able to generate a nest application', async () => {
      ...
      expect(e2eRsult.combinedOutput).toContain(
...
`
   ...
   it('should remove previous output before building with the --deleteOutputPath option set', async () => {
      ...
      runCLI(
         `generate @nx/node:app ${appName} --port=${port} --no-interactive`
      );
...
export class FooModel {
  ...
  bar?: number;
         ...
         const fooModelJs = readFile(
            `dist/libs/${nestlib}/src/lib/foo.model.js`
         );
