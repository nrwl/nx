...
| `--pattern` **[required]** | string | The pattern of files to look at. It can be a specific file, or any valid glob. Note: if using the CLI, globs with `**/*...` must be escaped with quotes | `**/*.stories.@(js|jsx|ts|tsx|md)` |
