...
It's been a few months now since our last major update with [Nx 20](/blog/announcing-nx-20), let's check in with the latest updates since 20 came out, as we've now dropped 3 minor releases since then. Here's a rundown of everything new:
...
Will create the necessary configuration for an existing targeted React or Vue application to work with RSBuild.
...
We've also added the capability to allow folks to authenticate their Powerpack license via Nx Cloud (rather than requiring that the license be committed to the repository). This should enable open source library authors use Powerpack - without exposing their private license via their repo.
...
- starting a local database whenever you start a backend application that depends on that database instance (while developing)
