...
We've been busy building the [Nx MCP server](https://nx.dev/features/enhance-AI), which gives LLMs deep access to your monorepo's structure. It helps AI tools to better understand your workspace architecture, browse the Nx docs and even trigger actions in your IDE like executing generators or visualizing the graph.
...
- `"continuous": true` marks the serve as a [continuous task](https://nx.dev/recipes/running-tasks/defining-task-pipeline#continuous-task-dependencies) so that will work properly in more complex task pipelines
...
There are a couple of things we want to configure. Check out [the comprehensive release documentation](https://nx.dev/features/manage-releases) to learn more about the different configuration options.
