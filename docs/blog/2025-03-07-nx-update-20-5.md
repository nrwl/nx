...
[See more details on how `@nx/js` infers tasks](/docs/technologies/typescript/introduction#how-nxjs-infers-tasks)
...
New Nx workspaces now default to React 19. Note that due to the extent of breaking changes coming with React 19, we will not be providing an automated migration to React 19 via [**`nx migrate`**](/docs/reference/nx-commands). If you're ready to make this migration on your own, [see our docs about using the new `reactCompiler`](/docs/technologies/react/guides/react-compiler)
