---
title: 'Nx 21 Release: Continous tasks and Terminal UI lead the way'
slug: nx-21-release
authors: ['Philip Fulcher']
tags: ['nx', 'release']
description: 'Discover the latest in Nx 20.5, featuring continuous tasks and the new Terminal UI'
...
- **Nx 21 Release: Continous tasks and Terminal UI lead the way**
- Continous tasks
- Terminal UI
- Migrate UI
...
- [Support for self-hosted caches](/blog/custom-runners-and-self-hosted-caching)
...
[Automated migrations](/features/automate-updating-dependencies) have always been a massively valuable part of Nx. Previously, we added migration docs￼ to better inform you of what’s happening during these migrations. Now, we’re bringing the migration process to the Nx Console so that you can step through migrations even easier than before.
...
As [announced previously](/blog/custom-runners-and-self-hosted-caching), custom runners have been removed in Nx 21. They are replaced by the [`preTasksExecution` and `postTaskExecution` hooks](/recipes/nx-release/updating-version-references). This removal impacts organizations by preventing older self-hosted distributed caches from working. However, we have provided [alternatives](/remote-cache) so that organizations of any size can [continue to use self-hosted caching solutions for free](/blog/custom-runners-and-self-hosted-caching) or migrate to [Nx Cloud](/nx-cloud).
...
For the rest of Launch Week, we’ll be doing deep dives on some of our biggest features. You won’t want to miss it, so keep an eye on our [socials](/community) and [YouTube channel](https://www.youtube.com/@nxdevtools/featured).
