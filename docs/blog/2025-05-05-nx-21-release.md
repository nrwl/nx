...
The `frontend:dev` task now depends on `api:serve`. We must also ensure the `api:serve` target is flagged as continuous. We've already flagged tasks as continuous if you're using [inferred tasks](/docs/concepts/inferred-tasks).
...
This is just the first release with these new APIs, and we're going to talk more about it and provide more examples in the coming months. [Check out the docs for more details.](/docs/guides/nx-release/updating-version-references)
...
Nx 21 [removes the deprecated custom task runners API](/docs/reference/deprecated/custom-tasks-runner), replacing it with the [`preTasksExecution` and `postTaskExecution` hooks](/docs/guides/nx-release/updating-version-references).
...
Read more about [the deprecated custom task runners API](/docs/reference/deprecated/custom-tasks-runner) and the [self-hosted caching options](/docs/guides/tasks--caching/self-hosted-caching) in our docs.
...
If you're [creating Nx plugins that infer tasks](/docs/extending-nx/project-graph-plugins), be sure they're migrated to `createNodesV2` to support newer versions of Nx.
