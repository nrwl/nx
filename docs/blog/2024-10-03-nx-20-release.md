...
- Added [`nx import`](nx-import) to bring import existing projects while maintaining git history.
- Added a [generated](crystalize-command) to convert all your Nx Plugins to use inferred tasks.
...
- A new [plugin for Gradle](nx-gradle) and support for [atomized tasks](gradle-atomizer)

With Nx 20 we're keeping the momentum going and bringing some quality improvments to the overall monorepo experience as well a boost in speed for TypeScript projects.
