...
Nx 22 is here and continues to expand support beyond the JavaScript ecosystem. This release brings **first-class support for .NET and Maven**. We want Nx to be your go-to solution for polyglot monorepos. This release also introduces pnpm catalog support, enhances our release workflows, and ships important stability improvements.
...
- [**Continuous tasks and Terminal UI**](/blog/nx-21-release): Configure long-running tasks like dev servers to start automatically when needed, with a new interface that keeps all your logs organized
- [**Angular 20, NestJS 11, and Storybook 9**](/blog/nx-21-2-release): Major framework updates with automatic migrations
- [**Enhanced AI integration**](/blog/whats-new-in-nx-self-healing-ci): The `configure-ai-agents` command and self-healing CI
- [**Redesigned graph UI**](/blog/nx-highlights-oct-2025): Composite mode by default with better controls and navigation
- [**Vite 7**](/blog/nx-highlights-oct-2025): Latest tooling support with automatic migration
- [**Playwright atomized test result merging**](/blog/nx-highlights-oct-2025): Bring together atomized test results in a single artifact
...
Following our success with the [Nx plugin for Gradle](/docs/technologies/java/gradle/introduction), we’re now bringing Maven support to Nx. Using Nx together with Maven provides Nx’s caching and task distribution to your Maven workspaces!

Adding Nx to an existing Maven project adds an `nx.json`, some `nx` wrapper binaries, and our Maven Plugin (`dev.nx.maven.nx-maven-plugin`) to the root `pom.xml`. The `@nx/maven` plugin automatically reflects Maven's modules and lifecycle into Nx's project graph. As your Maven modules and goals evolve, Nx will automatically pick it up without maintaining any additional configuration.
...
Like our other plugins, Maven support includes intelligent caching and distributed execution. Your Maven builds will benefit from Nx’s computation caching, which skips unnecessary rebuilds when nothing has changed. For teams using Nx Cloud, this means dramatically faster CI pipelines for your Java projects.
...
The plugin uses your existing `.csproj`, `.fsproj`, or `.vbproj` files while integrating with Nx’s task graph. This means you get intelligent caching for your .NET builds, distributed task execution on Nx Cloud, and accurate dependency tracking between your .NET projects and any JavaScript projects in your workspace.
...
We’ve significantly improved `nx release` based on feedback from teams using it in production. Several important changes make the release process more intuitive and flexible and progress us along our journey to fully supporting application releases alongside our powerful existing options for libraries.
