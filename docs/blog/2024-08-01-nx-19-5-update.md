...
Nx now has support for [Stackblitz](https://stackblitz.com/) (again). For a while it was not possible to use Nx on Stackblitz because we implemented some perf critical parts of Nx as native components with Rust. Huge shoutout to the Stackblitz team and in particular [LongYinan](https://github.com/Brooooooklyn) for [their amazing work of pushing napi-rs](https://napi.rs/) forward and helping us [add WebAssembly support to Nx](https://github.com/nrwl/nx/pull/22870).
...
[Nx release](/nx-api/nx/documents/release) now supports ["version plans"](https://github.com/nrwl/nx/pull/23190) whether the work they've done would qualify as a `major`/`minor`/`patch`/etc. as well as providing a changelog message for their work:
...
This feature is **still experimental**, but we are actively using and improving this with some of our larger clients and partners in the open source community. To learn more about [Nx Release, check out our docs](/features/manage-releases).
