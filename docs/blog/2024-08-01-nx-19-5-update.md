...
We just added a new [Hobby Tier](/nx-cloud#plans#plan-details) to Nx Cloud! Why is this exciting? It is completely free and contains all major features, which includes the ability to [distribute your tasks with Nx Agents](/docs/features/ci-features/distribute-task-execution).
...
Checkout the [plan details page](/nx-cloud#plans#plan-details) for more info, and see how Nx Cloud can help you! And in case you missed it, check out our latest blog on the latest Nx Cloud feature: [Explain with AI](/blog/explain-with-ai)!
...
Nx Cloud has long offered [flaky task detection](/docs/features/ci-features/flaky-tasks) on CI, and now this capability is available **even when running them locally**.
...
In addition, we've added a callout for tasks that [leverage the `Nx Atomizer`](/docs/features/ci-features/split-e2e-tasks). Since atomizer tasks are designed to be used with [Nx Agents](/docs/features/ci-features/distribute-task-execution#distribute-task-execution-nx-agents), this callout will appear in yellow to warn if your project is not yet configured to run them on CI, and you can click on this callout for an easy way to complete that setup:
...
Note that the new, free [hobby tier for Nx Cloud](#new-nx-cloud-hobby-tier) includes the ability to run [Nx Agents](/docs/features/ci-features/distribute-task-execution) without the need to immediately jump onto a paid plan.

## Pattern Support for `targetDefaults`

When using the [Atomizer features](/docs/features/ci-features/split-e2e-tasks) of plugins like [`@nx/playwright`](/docs/technologies/test-tools/playwright/introduction) and [`@nx/cypress`](/docs/technologies/test-tools/cypress/introduction), you end up creating tasks with dynamic but predictable names.
...
Read more about [reducing configuration with `targetDefaults`](/docs/guides/tasks--caching/reduce-repetitive-configuration) and [how you can define task pipelines with `targetDefaults`](/docs/features/run-tasks#defining-a-task-pipeline).
...
To address this, all tasks now support a [`parallelism` property](/docs/reference/project-configuration#parallelism). By setting this property to `false`, you can instruct the Nx task runner not to run a specific task in parallel. This allows us to define parallelism as a task property, making our CI configuration more declarative.
...
Learn more about how you can [Parallelize Tasks Across Multiple Machines Using Nx Agents](/docs/features/ci-features/split-e2e-tasks):
...
Nx now supports incremental builds with our [`vite` plugin](/docs/technologies/build-tools/vite/introduction). Incremental builds in Nx workspaces allow you to build any package in your workspace individually, and will then automatically use that built artifact when then building any project or application that consumes the package. This way you can speed up your build and CI times through optimizations like [only building packages that were affected](/docs/reference/nx-commands), using [Nx Replay](/docs/features/ci-features/remote-cache) to effortlessly cache and share your built artifacts, and allowing you to run builds of your various packages in parallel using [Nx Agents](/docs/features/ci-features/distribute-task-execution).

Read more about [buildable libraries and incremental builds](/docs/concepts/buildable-and-publishable-libraries).

## Project Crystal Conversion Generators

Since launching [Nx Project Crystal](/docs/concepts/inferred-tasks) in Nx version 18, we've adjusted our generators to leverage these features when creating new workspaces and projects. We've now added the `convert-to-inferred` generator to give you an automated way to opt into project crystal.
...
The important thing to note here is that while this generator will make adjustments to the `project.json` file of the projects you convert, the behavior of your tasks should remain the same. You can see project details via the [Nx Console](/docs/getting-started/editor-setup) plugin of your IDE, or in a browser by running the command: `nx show project [projectName]`.

In Nx 19, we were focused on adding `convert-to-inferred` to our [Playwright](/docs/technologies/test-tools/playwright/introduction), [Cypress](/docs/technologies/test-tools/cypress/introduction), and [ESLint](/docs/technologies/eslint/introduction) plugins - where bringing the [Nx Atomizer](/docs/features/ci-features/split-e2e-tasks) provided the greatest value. Since then, we've added this generator to most other plugins. Currently, our React native plugins (react native, expo, detox) are the only plugins that are outstanding, and we'll be providing `convert-to-inferred` generators here in the near future.

## Gradle Composite Builds Support

In [Nx 19.0](/blog/nx-19-release), we introduced support for Gradle through our [Gradle plugin](/docs/technologies/java/introduction). With the latest update, this plugin now also supports [Gradle Composite builds](https://docs.gradle.org/current/userguide/composite_builds.html), in addition to multi-project builds.
...
The new [`@nx/gradle` plugin](/docs/technologies/java/introduction) now includes experimental support for [Test Atomization](/docs/features/ci-features/split-e2e-tasks).
...
Currently with [Nx release](/docs/reference/nx-commands), you already have two different ways of figuring out the how to modify the current version of a package to get its updated version:
...
When your team is ready to release, you can now simply run the CLI command `nx release` (or use the [powerful programmatic API](/docs/features/manage-releases#using-the-programmatic-api-for-nx-release)) and nx will ingest all the outstanding version plan files and use the combined information to determine the resulting version for all referenced projects and/or release groups as part of the versioning, changelog generation, and publishing steps. (You can also run the command `nx release version` if all you want to do is update the version!)

![Nx Release With Plans](/blog/images/2024-08-01/nx-release-with-plans.avif)

Anyone who has used the tools "Changesets" or "Beachball" will be familiar with the concept of file based versioning, but with Nx release you get the power to combine one or more of the above mentioned versioning strategies all within the same workspace for different release groups, and we are really excited to keep building out this feature set and mark it as non-experimental (by fully documenting it) very soon. To learn more about the other features of [Nx Release in the meantime, check out our docs](/docs/features/manage-releases).
...
Our `@nx/react` package will now create new React applications using version 18.3, and we now support the new experimental `reactCompiler`. Users can install the [`babel-plugin-react-compiler` package](https://www.npmjs.com/package/babel-plugin-react-compiler) and you can follow [this guide for how to enable it with Nx](/docs/technologies/react/guides/react-compiler).

Note that due to the extent of breaking changes coming with React 19, we will not be providing a automated migration to React 19 via [`nx migrate`](/docs/reference/nx-commands).

## Automatically Update Nx

As always - updating Nx and its plugins is easy as we ship an [automated migration command](/docs/features/automate-updating-dependencies).
