...
If you're using [inferred tasks](/concepts/inferred-tasks), or running Cypress directly with the `nx:run-commands` executor, you can [provide the Cypress args](/recipes/running-tasks/pass-args-to-commands) for the command you're running.

If you're using the `@nx/cypress:cypress` executor, you can provide [the options the executor accepts](/nx-api/cypress/executors/cypress#options).
