...
{% cards cols="2" %}
{% card title="Official @nx/vue plugin" type="document" url="/docs/technologies/vue/introduction" /%}
{% card title="Module Federation Improvements" type="document" url="/docs/technologies/module-federation/concepts/module-federation-and-nx" /%}
...
{% card title="Each task has its own cache property" type="document" url="/docs/reference/project-configuration" /%}
{% /cards %}
...
{% cards cols="2" %}
{% card title="Rename @nx/linter to @nx/eslint" type="document" url="/docs/reference/deprecated/rescope#rename" /%}
...
{% card title="Removed ability for Angular CLI to use our generators and executors" type="external" url="/docs/reference/deprecated/angular-schematics-builders" /%}
