...
(async () => {
  ...
  if (options.clearLocalRegistry) {
    execSync('pnpm run local-registry clear');
  }
...
function parseArgs() {
  const parsedArgs = yargs
    .scriptName('pnpm run nx-release')
    .wrap(144)
    .strictOptions()
    .version(false)
    .command(
    ...
    .check((args) => {
      ...
      if (!args.local) {
        ...
        if (!args.force && registryIsLocalhost) {
          throw new Error(
            'Registry is still set to localhost! Run "pnpm run local-registry disable" or pass --force'
          );
        }
      } else {
        if (!args.force && !registryIsLocalhost) {
