...
const { CopyAssetsHandler } = require('@nx/js/src/utils/assets/copy-assets-handler');
...
const taskName = args.find(arg => arg.startsWith('--task='))?.split('=')[1] || 'build';

if (!packageName) {
  console.error('Error: Package name is required');
  console.error('Usage: node scripts/copy-assets.js <package-name> [--watch] [--task=taskname]');
  process.exit(1);
}
...
function printAssetDetails(assets) {
  ...
  assets.forEach((asset, index) => {
    if (typeof asset === 'string') {
      ...
      console.log(`  ${index + 1}. Input: "${asset.input}", Glob: "${asset.glob}", Output: "${asset.output}"`);
      if (asset.ignore) {
...
async function main() {
  try {
    ...
    const projectJsonPath = path.join(workspaceRoot, 'packages', packageName, 'project.json');
    ...
    if (projectJson.targets?.[taskName]?.options?.assets || projectJson.targets?.[taskName]?.assets) {
      assets = projectJson.targets[taskName].options?.assets || projectJson.targets[taskName].assets;
      outputPath = projectJson.targets[taskName].options?.outputPath || projectJson.targets[taskName].outputPath;
    ...
    if (!assets) {
      ...
      for (const task of commonTasks) {
        if (projectJson.targets?.[task]?.options?.assets || projectJson.targets?.[task]?.assets) {
          console.log(`Found assets in task: ${task}`);
          assets = projectJson.targets[task].options?.assets || projectJson.targets[task].assets;
          outputPath = projectJson.targets[task].options?.outputPath || projectJson.targets[task].outputPath;
          foundTaskName = task;
          break;
        }
      }
    ...
    if (!assets || assets.length === 0) {
      ...
      console.log('Available tasks with options:', 
    ...
    if (!outputPath) {
      try {
        const tsconfigLibPath = path.join(workspaceRoot, 'packages', packageName, 'tsconfig.lib.json');
        if (fs.existsSync(tsconfigLibPath)) {
          ...
          if (outDir) {
            ...
            outputPath = path.resolve(path.join(workspaceRoot, 'packages', packageName), outDir);
        ...
        console.warn(`Could not read tsconfig.lib.json, using default: ${error.message}`);
        outputPath = path.join(workspaceRoot, 'dist', 'packages', packageName);
      }
    }
