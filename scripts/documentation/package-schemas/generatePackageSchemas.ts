   ...
   const packages = findPackageMetadataList(absoluteRoot, 'packages').map(
      (packageMetadata) => {
         ...
         if (!!packageMetadata.executors.length) {
            packageMetadata.executors = packageMetadata.executors.map(
               (item) => ({
                  ...item,
                  schema: processSchemaData(
                     item.schema as NxSchema,
                     getCurrentSchemaPath(
                        item['path'].replace('schema.json', '')
                     )
                  ),
               })
            );
         ...
         if (!!packageMetadata.generators.length) {
            packageMetadata.generators = packageMetadata.generators.map(
               (item) => ({
                  ...item,
                  schema: processSchemaData(
                     item.schema as NxSchema,
      ...
      p.documents.forEach((d) =>
         fileGenerationPromises.push(
            generateMarkdownFile(
               join(outputPackagesPath, p.name, 'documents'),
               {
                  name: d.id,
                  template: d.content,
               }
