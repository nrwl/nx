...
async function generate() {
  ...
  Frameworks.forEach((framework) => {
    execSync(
      `rm -rf docs/${framework}/api-nx-devkit && npx typedoc packages/devkit/index.ts packages/devkit/ngcli-adapter.ts --tsconfig packages/devkit/tsconfig.lib.json --out ./docs/${framework}/api-nx-devkit --hideBreadcrumbs true --publicPath /latest/${framework}/nx-devkit/ --theme dist/typedoc-theme/src/lib`
    );
  });
