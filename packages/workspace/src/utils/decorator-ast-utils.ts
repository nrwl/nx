...
function _getImportsFromNode(
  ...
  if (!modulePath.startsWith(`${npmScope}/`)) {
    return {};
  }
...
export function getDecoratorMetadata(
  ...
  const npmScope = module.split('/')[0];
  const imports: { [name: string]: string } = findNodes(
    source,
    ts.SyntaxKind.ImportDeclaration
  )
    .map((node: ts.ImportDeclaration) =>
      _getImportsFromNode(node, source, npmScope)
    )
    .reduce(
      (
    ...
    .filter((expr) => {
      if (expr.expression.kind == ts.SyntaxKind.Identifier) {
        ...
        return (
          ...
          imports[id.getFullText(source)] === module
        );
      } else if (
        ...
        return id === identifier && imports[moduleId + '.'] === module;
      }
...
function _addSymbolToModuleMetadata(
  source: ts.SourceFile,
  modulePath: string,
  ...
  const nodes = getDecoratorMetadata(source, identifier, moduleName);
  ...
  const insert = new InsertChange(modulePath, position, toInsert);
  return [insert];
}
...
export function addImportToModule(
  ...
  return _addSymbolToModuleMetadata(
    source,
    modulePath,
    'imports',
    symbolName,
