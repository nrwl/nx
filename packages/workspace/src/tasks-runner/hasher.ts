...
export class Hasher {
  ...
  private async runtimeInputsHash(): Promise<RuntimeHashResult> {
    ...
    if (inputs.length > 0) {
      try {
        ...
        const value = await hasha(values.map(v => v.value), {
          algorithm: 'sha256'
        });
  ...
  private async implicitDepsHash(): Promise<ImplicitHashResult> {
    ...
    this.implicitDependencies = Promise.resolve().then(async () => {
      ...
      const combinedHash = await hasha(fileHashes.map(v => v.hash), {
        algorithm: 'sha256'
      });
      return {
        value: combinedHash,
...
export class ProjectHashes {
  ...
  async hashProject(
    ...
    return Promise.resolve().then(async () => {
      ...
      const value = await hasha([...depHashes.map(d => d.value), projectHash]);
