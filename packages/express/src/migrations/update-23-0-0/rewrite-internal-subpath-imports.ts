...
const FROM_PREFIX = '@nx/express/src/';
const TO_PUBLIC = '@nx/express';
const TO_INTERNAL = '@nx/express/internal';

// Symbols exported from `@nx/express`'s public entry (packages/express/index.ts).
// A named import/export of one of these from `@nx/express/src/*` is routed to
// the public `@nx/express` entry; everything else goes to `@nx/express/internal`.
const PUBLIC_SYMBOLS: ReadonlySet<string> = new Set(['applicationGenerator']);
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/express/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/express, internals to @nx/express/internal).`
    );
  }
 ...
 * that resolve to `@nx/express`'s public entry and the ones that don't. If both
