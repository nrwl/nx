...
import {
  ...
  NodeBuildEvent
} from '../../../builders/src/node/build/node-build.builder';
...
export default class DeployBuilder implements Builder<DeployArgs> {
  ...
  run(builderConfig: BuilderConfiguration<DeployArgs>): Observable<BuildEvent> {
    const [
      ...
      target,
      configuration
    ] = builderConfig.options.buildTarget.split(':');

    const buildBuilderConfig = this.context.architect.getBuilderConfiguration<
      BuildNodeBuilderOptions
    >({
      project,
      target,
      ...
      .pipe(
        ...
        tap(r => {
          if (r.success) {
            ...
            let mainJs = readFileSync(
              join(tmpWithProject, 'main.js')
            ).toString();
            ...
            const dir = readdirSync(
              join(buildBuilderConfig.sourceRoot, 'azure')
            )[0];
            execSync(
              `cp -r ${join(
                buildBuilderConfig.sourceRoot,
                'azure',
                dir
              )}/*.* ${tmpWithProject}`
            );
            ...
            execSync(
              `git push --force ${createGitUrl(
                builderConfig.options.deployment.remote,
                builderConfig.options.username,
                builderConfig.options.password
              )} master`,
              {
                cwd: tmpWithProject,
                stdio: [0, 1, 2]
            ...
            const app = apps.find(
              f => f.name === builderConfig.options.azureWebAppName
            );
  ...
  private createApp(builderConfig: BuilderConfiguration<DeployArgs>) {
    const resource = JSON.parse(
      execSync(`az appservice plan list`).toString()
    )[0];
    ...
    execSync(
      `az webapp create --name=${
        builderConfig.options.azureWebAppName
      } --plan=${plan} --resource-group=${resourceGroup} --runtime='${runtime}' --deployment-local-git`,
      { stdio: [0, 1, 2] }
    );
...
function createGitUrl(
  ...
  const [protocol, url] = remote.split('://');
  return `${protocol}://${username.replace(/@/g, '%40')}:${password.replace(
    /@/g,
    '%40'
  )}@${url}`;
}
