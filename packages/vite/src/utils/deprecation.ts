...
export const NX_VITE_TS_PATHS_DEPRECATION_MESSAGE =
  'The `nxViteTsPaths` plugin from `@nx/vite/plugins/nx-tsconfig-paths.plugin` is deprecated and will be removed in Nx v24. Replace it with `tsconfigPaths()` from the `vite-tsconfig-paths` package. See https://nx.dev/technologies/build-tools/vite/api for details.';

export const NX_COPY_ASSETS_PLUGIN_DEPRECATION_MESSAGE =
  "The `nxCopyAssetsPlugin` plugin from `@nx/vite/plugins/nx-copy-assets.plugin` is deprecated and will be removed in Nx v24. Use Vite's native `publicDir` option or the `vite-plugin-static-copy` package instead. See https://nx.dev/technologies/build-tools/vite/api for details.";
