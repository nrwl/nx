   ...
   describe('--all', () => {
      it('should successfully migrate a project using Vite executors to plugin', async () => {
         ...
         if (typeof hasVitePlugin !== 'string') {
            [
               ...
               expect(hasVitePlugin.options[targetOptionName]).toEqual(
                  targetName
               );
            });
         }
      ...
      it('should handle multiple different target names for the same project', async () => {
         ...
         const vitePluginRegistrations = nxJsonPlugins.filter(
            (
               plugin
            ): plugin is ExpandedPluginConfiguration<VitePluginOptions> =>
               typeof plugin !== 'string' && plugin.plugin === '@nx/vite/plugin'
         );
         expect(vitePluginRegistrations.length).toBe(4);
         expect(vitePluginRegistrations[0].options.buildTargetName).toBe(
            'build'
         );
         expect(vitePluginRegistrations[0].options.serveTargetName).toBe(
            'serve'
         );
         ...
         expect(vitePluginRegistrations[1].options.buildTargetName).toBe(
            'build'
         );
         ...
         expect(vitePluginRegistrations[3].options.serveTargetName).toBe(
            'serve'
         );
