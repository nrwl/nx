   ...
   forEachExecutorOptions<JestExecutorOptions>(
      ...
      (value, proj, targetName, configuration) => {
         ...
         if (!configuration) {
            // Options
            if (
               value.passWithNoTests === jestDefaults.options.passWithNoTests
            ) {
            ...
            if (
               value.codeCoverage ===
               jestDefaults.configurations.ci.codeCoverage
            ) {
            ...
            if (
               Object.keys(projConfig.targets[targetName].configurations)
                  .length === 0
            ) {
