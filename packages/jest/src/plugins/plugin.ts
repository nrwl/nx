...
function getInputs(
  ...
  const presetInput = disableJestRuntime
    ? resolvePresetInputWithoutJestResolver(preset, projectRoot)
...
function resolvePresetInputWithoutJestResolver(
  ...
  return presetValue.startsWith('..')
    ? join('{workspaceRoot}', join(projectRoot, presetValue))
    : join('{projectRoot}', presetValue);
}
...
async function getJestOption<T = any>(
  ...
  if (rawConfig.preset) {
    ...
    try {
      const preset = await loadConfigFile(presetPath);
      if (preset[optionName]) return preset[optionName];
    } catch {
      throw new Error(`Could not load preset file: ${presetPath}`);
    }
  }
