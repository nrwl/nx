...
const FROM_PREFIX = '@nx/remix/src/';
const TO_PUBLIC = '@nx/remix';
const TO_INTERNAL = '@nx/remix/internal';

// Symbols exported from `@nx/remix`'s public entry (packages/remix/index.ts).
// A named import/export of one of these from `@nx/remix/src/*` is routed to
// the public `@nx/remix` entry; everything else goes to `@nx/remix/internal`.
const PUBLIC_SYMBOLS: ReadonlySet<string> = new Set(['createWatchPaths']);
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/remix/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/remix, internals to @nx/remix/internal).`
    );
  }
 ...
 * that resolve to `@nx/remix`'s public entry and the ones that don't. If both
