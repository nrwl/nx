...
const FROM_PREFIX = '@nx/expo/src/';
const TO_PUBLIC = '@nx/expo';
const TO_INTERNAL = '@nx/expo/internal';

// Symbols exported from `@nx/expo`'s public entry (packages/expo/index.ts).
// A named import/export of one of these from `@nx/expo/src/*` is routed to
// the public `@nx/expo` entry; everything else goes to `@nx/expo/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/expo/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/expo, internals to @nx/expo/internal).`
    );
  }
 ...
 * that resolve to `@nx/expo`'s public entry and the ones that don't. If both
