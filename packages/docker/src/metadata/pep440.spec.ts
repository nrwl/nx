...
/**
 * Parity suite: compares this module's output directly against the real
 * `@renovatebot/pep440@5` package for a broad set of version strings.
 *
 * This package is a test-only devDependency here — it is NOT used at
 * runtime; `resolveVersion.ts` only imports the local `./pep440` module.
 * Install with: npm install --save-dev @renovatebot/pep440@5
 *
 * If `@renovatebot/pep440` isn't installed, this suite is skipped rather
 * than failing, so it doesn't block running the rest of the tests.
 */
describe('parity with @renovatebot/pep440@5', async () => {
  let reference: typeof import('@renovatebot/pep440') | null = null;
  try {
    reference = await import('@renovatebot/pep440');
  } catch {
    reference = null;
  }
  ...
  for (const v of versions) {
    test(`valid("${v}") matches reference`, (t) => {
      if (!reference) return t.skip('@renovatebot/pep440 not installed');
      assert.equal(valid(v), reference.valid(v));
    });

    test(`clean("${v}") matches reference`, (t) => {
      if (!reference) return t.skip('@renovatebot/pep440 not installed');
      assert.equal(clean(v), reference.clean(v));
    });

    test(`explain("${v}") matches reference`, (t) => {
      if (!reference) return t.skip('@renovatebot/pep440 not installed');
      assert.deepEqual(explain(v), reference.explain(v));
    });
  }

  for (const v of ['1.2.3', '1.2.3.4', '5', '5.1', '2!1.2.3a1']) {
    test(`major/minor/patch("${v}") match reference`, (t) => {
      if (!reference) return t.skip('@renovatebot/pep440 not installed');
      assert.equal(major(v), reference.major(v));
      assert.equal(minor(v), reference.minor(v));
      assert.equal(patch(v), reference.patch(v));
    });
  }
});
