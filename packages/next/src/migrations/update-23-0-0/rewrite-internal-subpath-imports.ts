...
const FROM_PREFIX = '@nx/next/src/';
const TO_PUBLIC = '@nx/next';
const TO_INTERNAL = '@nx/next/internal';

// Symbols exported from `@nx/next`'s public entry (packages/next/index.ts).
// A named import/export of one of these from `@nx/next/src/*` is routed to
// the public `@nx/next` entry; everything else goes to `@nx/next/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/react/src/* imports in ${touchedCount} file(s) ` +
