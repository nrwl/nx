...
describe('convertToCypressTen', () => {
   ...
   describe('convertCypressProject', () => {
      ...
      it('should update project w/defaults', async () => {
         ...
         expect(
            tree.read('app-e2e/cypress.config.ts', 'utf-8')
         ).toMatchSnapshot();
      ...
      it('should handle sharing the same config across projects', async () => {
         ...
         appOneProjectConfig.targets['e2e'].options.cypressConfig =
            'cypress.json';
         updateProjectConfiguration(tree, 'app-e2e', appOneProjectConfig);
         const appTwoProjectConfig = readProjectConfiguration(
            tree,
            'app-two-e2e'
         );
         appTwoProjectConfig.targets['e2e'].options.cypressConfig =
            'cypress.json';
...
`
      ...
      it('should move files to the new integration folder (e2e/)', () => {
         ...
         const oldIntegrationFolderContents =
            tree.children(oldIntegrationFolder);

         updateProjectPaths(tree, projectConfig, cypressConfigs);

         const newIntegrationFolderContents =
            tree.children(newIntegrationFolder);
...
`
         ...
         const actual = updatePluginFile(
            ...
            { root: 'app-e2e' },
            {
               cypressConfigJson: {},
               cypressConfigTs: {
                  e2e: { pluginsFile: './src/plugins/index.ts' },
               },
            }
         );
...
`;
         ...
         const actual = updatePluginFile(
            ...
            { root: 'app-e2e' },
            {
               cypressConfigJson: {},
               cypressConfigTs: {
                  e2e: { pluginsFile: './src/plugins/index.js' },
               },
            }
         );
