...
import { addLinterToCyProject } from './add-linter';

describe('addLinterToCyProject', () => {
  let tree: Tree;
  ...
  it('adds the projectService block to an existing flat config when typed linting is requested', async () => {
    // A pre-existing eslint config means lintProjectGenerator is skipped, so the
    ...
    await addLinterToCyProject(tree, {
      project: 'myapp-e2e',
      linter: 'eslint',
  ...
  it('does not append a projectService block to a config that already uses the legacy parserOptions.project shape', async () => {
    // A project generated by an older Nx has the legacy shape. Appending the
    // projectService block alongside it makes typescript-eslint v8 throw, so the
    // helper must leave the existing typed-linting config in place.
    tree.write(
      'apps/myapp-e2e/eslint.config.mjs',
