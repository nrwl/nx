...
const FROM_PREFIX = '@nx/plugin/src/';
const TO_PUBLIC = '@nx/plugin';
const TO_INTERNAL = '@nx/plugin/internal';

// Symbols exported from `@nx/plugin`'s public entry (packages/plugin/index.ts).
// A named import/export of one of these from `@nx/plugin/src/*` is routed to
// the public `@nx/plugin` entry; everything else goes to `@nx/plugin/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/plugin/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/plugin, internals to @nx/plugin/internal).`
    );
  }
 ...
 * that resolve to `@nx/plugin`'s public entry and the ones that don't. If both
