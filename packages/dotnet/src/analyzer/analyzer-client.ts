...
/**
 * Whether a cached analysis still reflects what is on disk: both the files the
 * evaluation read and, for projects that split, the sources those targets came
 * from.
 */
async function isStillValid(
  ...
  return (
    ...
    (await isSourceHashCurrent(entry.sourceHash, entry.result))
  );
}
...
export async function analyzeProjects(
  ...
  try {
    ...
    analyzerCache.set(filesHash, { result, inputsHash, sourceHash });
