...
public static partial class TargetBuilder
{
        ...
        string fileName)
    {
        ...
        // Every unit's results directory is this same project-level base with
        // only the unit id appended, so whether it can be expressed as an Nx
        // path is a property of the project, decided once here rather than
        // per unit below.
        ...
        foreach (var unit in units)
        {
            ...
            // Forwarding is what lets flags typed on the parent
            // (`nx test-ci my-proj --framework=net8.0`) reach each task.
    ...
    /// fails and no output looks wrong; the split run just stops covering
    /// them. Only fires when coverage is affected.
    /// </remarks>
