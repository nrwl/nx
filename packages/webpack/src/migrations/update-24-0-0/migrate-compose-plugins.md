# Migrate webpack compose helpers

Migrate this workspace off `composePlugins`, `composePluginsSync`, `withNx`,
`withWeb`, and webpack `withReact`. Their exports remain available for migration
...
- Preserve the installed webpack version, bundler, targets, configurations, and
  commands. Executor conversion is a separate migration. Handle both existing
  executors and targets already using webpack CLI.
...
Use a standard webpack configuration with `NxAppWebpackPlugin` from
`@nx/webpack/app-plugin`. For React apps also use `NxReactWebpackPlugin` from
`@nx/react/webpack-plugin`. Do not add React configuration to Node or plain web
apps. Do not change frameworks or upgrade dependencies to simplify the rewrite.

Map the effective old options to these plugins and standard webpack properties.
...
and webpack CLI resolve relative to the project/config working directory. Preserve
...
When keeping `@nx/webpack:webpack`, set `standardWebpackConfigFunction: true` for a
config exported as a standard function; otherwise the executor can pass Nx context
instead of webpack's `(env, argv)`. Preserve explicit target configuration options.
...
`@nx/webpack:convert-config-to-webpack-plugin` can provide a starting point, but it
retains `webpack.config.old.*` through `useLegacyNxPlugin`. It also rejects some
...
An `apply` object belongs in `config.plugins`; exporting it as the whole webpack
config is invalid. A top-level config function must return a config object (or
supported config array), not a plugin.

Do not treat adapters as equivalent by default. Webpack processes some options
