         ...
         resolveShare: (args) => {
            ...
            args.resolver = function () {
               if (!runtimeStore.sharedPackagesFromDev[pkgName]) {
                  ...
                  const devRemoteInstanceToUse =
                     GlobalFederation.__INSTANCES__.find(
                        (instance) =>
                           instance.options.shared[pkgName] &&
                           runtimeStore.devRemotes.find(
                              (dr) => instance.name === dr
                           )
                     );
               ...
               try {
                  const remotePkgInfo = remoteInstance.options.shared[
                     pkgName
                  ].find((shared) => shared.from === remoteInstanceName);
                  remotePkgInfo.useIn.push(runtimeStore.name);
                  remotePkgInfo.useIn = Array.from(
                     new Set(remotePkgInfo.useIn)
                  );
