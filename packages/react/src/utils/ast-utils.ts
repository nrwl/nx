...
export function addInitialRoutes(
   ...
   const outerMostJsxClosing =
      jsxClosingElements[jsxClosingElements.length - 1];
...
export function getComponentPropsInterface(
   ...
   if (tsModule.isFunctionDeclaration(cmpDeclaration)) {
      const propsParam: ts.ParameterDeclaration =
         cmpDeclaration.parameters.find(
            (x) =>
               tsModule.isParameter(x) &&
               (x.name as ts.Identifier).text === 'props'
         );
