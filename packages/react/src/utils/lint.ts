...
/**
 * If the user is creating a JS-based react project we need to make
 * sure that the root ESLint configuration has an entry for JS in
 * its overrides array.
 */
export function updateRootESLintConfig({ js }: { js: boolean }): Rule {
  if (!js) {
    return noop();
  }
...
export const createReactEslintJson = (options: {
  ...
  const generalCodeConfig = createGeneralCodeConfig({
    js: options.js,
    parserOptionsProject: options.parserOptionsProject,
  });
  const jsxSpecificConfig = createJSXSpecificConfig();

  /**
   * For the JS use-case within Nx, all code and JSX code is written within .js files,
   * so we combine the relevant config into a single overrides.
   */
  if (options.js) {
    const {
      plugins: generalCodePlugins,
      parserOptions: generalCodeParserOptions,
      env: generalCodeEnv,
      ...
      overrides: [
        {
          ...
          parserOptions: generalCodeParserOptions,
          settings: jsxSpecificSettings,
          plugins: [...generalCodePlugins, ...jsxSpecificPlugins],
          env: generalCodeEnv,
          rules: {
            ...generalCodeRules,
            ...jsxSpecificRules,
...
function createGeneralCodeConfig(options: {
  js: boolean;
  parserOptionsProject: string[];
}) {
  if (!options.js && !options.parserOptionsProject) {
    throw new Error('parserOptionsProject must be specified if not JS');
  }
