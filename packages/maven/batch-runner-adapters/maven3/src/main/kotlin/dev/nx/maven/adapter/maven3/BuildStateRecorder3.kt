 ...
 * Maven 3.x version.
 */
object BuildStateRecorder3 {
    private const val BUILD_STATE_FILE = "nx-build-state.json"
    private val log: Logger = LoggerFactory.getLogger(BuildStateRecorder3::class.java)
    ...
    fun recordBuildState(project: MavenProject) {
        ...
        val pomFile = if (currentPomFile != null && currentPomFile.canonicalPath != defaultPomFile.canonicalPath) {
            ...
            log.info("Captured non-default pomFile: $relativePath")
            relativePath
        } else {
            null
        }
