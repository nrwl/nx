...
function replaceRequireCalls(
   ...
   const mfUtilTokens = mfUtilRequires.map((node) => {
      ...
      const mfTokens = allTokens.filter((node) =>
         MODULE_FEDERATION_PUBLIC_TOKENS.some(
            (token) => node.getText() === token
         )
      );
   ...
   changes.push({
      ...
      content: `\nconst { ${mfUtilTokens
         .map((mfUtilToken) =>
            mfUtilToken.mfTokens.map((node) => node.getText())
         )
   ...
   changes.push({
      ...
      content: `\nimport { ${mfUtilsWithMultipleImports
         .map((importDeclaration) =>
            importDeclaration.mfImportSpecifierNodes.map((node) =>
               node.getText()
            )
         )
