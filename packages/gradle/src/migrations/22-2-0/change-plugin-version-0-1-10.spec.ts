...
import update from './change-plugin-version-0-1-10';
import { gradleProjectGraphPluginName } from '../../utils/versions';

describe('change-plugin-version-0-1-10 migration', () => {
  let tempFs: TempFs;
  let cwd: string;
  let tree: Tree;
  ...
  it('should update plugin version to 0.1.10 in Groovy DSL', async () => {
    ...
    expect(content).toContain(
      `id "${gradleProjectGraphPluginName}" version "0.1.10"`
    );
    expect(content).not.toContain('version "0.0.1"');
  });

  it('should update plugin version to 0.1.10 in Kotlin DSL', async () => {
    await tempFs.createFiles({
      'nx.json': JSON.stringify({
        plugins: ['@nx/gradle'],
      }),
  ...
  it('should not update if nx.json is missing', async () => {
    ...
    expect(content).not.toContain('version "0.1.10"');
  });
...
`,
    ...
    expect(catalogContent).toContain('nx-project-graph = "0.1.10"');
...
`,
    ...
    const catalogContent = tree.read('proj/gradle/libs.versions.toml', 'utf-8');
    expect(catalogContent).toContain('version = "0.1.10"');
...
`,
    ...
    expect(catalogContent).toContain(
      `"${gradleProjectGraphPluginName}:0.1.10"`
    );
...
`,
    ...
    expect(catalogContent).toContain('nx-gradle-project-graph = "0.1.10"');
