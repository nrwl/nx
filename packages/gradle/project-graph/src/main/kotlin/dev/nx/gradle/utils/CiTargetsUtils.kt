  ...
  return try {
    ...
    topLevelClasses.forEach { classDecl ->
      ...
      if (!classDecl.isInterface) {
        ...
        // Check for direct nested classes with @Nested annotation (also excluding interfaces)
        val nestedTestClasses =
            classDecl.members.filterIsInstance<ClassOrInterfaceDeclaration>().filter {
              !it.isInterface && hasNestedAnnotation(it)
            }
...
// Shared compiler configuration to avoid recreation
private val sharedCompilerConfiguration by lazy { CompilerConfiguration() }

// Singleton PsiManager to reuse across all Kotlin file parsing
private object KotlinPsiManager {
  private val disposable = Disposer.newDisposable()
  private val environment =
      KotlinCoreEnvironment.createForProduction(
          disposable, sharedCompilerConfiguration, EnvironmentConfigFiles.JVM_CONFIG_FILES)
...
internal fun parseKotlinFileWithAst(
  ...
  return try {
    ...
    // Create virtual file and parse it using singleton PsiManager (with timeout protection)
    val ktFile =
        try {
          synchronized(KotlinPsiManager) {
            val virtualFile = LightVirtualFile(file.name, KotlinFileType.INSTANCE, content)
            KotlinPsiManager.psiManager.findFile(virtualFile) as? KtFile
          }
        } catch (e: Exception) {
          logger?.warn(
...
fun getAllVisibleClassesWithNestedAnnotation(
  ...
  return when {
    ...
    file.name.endsWith(".kt") -> {
      ...
      val astResult = parseKotlinFileWithAst(file, logger)
      if (astResult != null) {
