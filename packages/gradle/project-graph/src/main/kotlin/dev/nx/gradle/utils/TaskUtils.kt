...
fun processTask(
  ...
  if (inputs != null && inputs.isNotEmpty()) {
    logger.info("${task}: processed inputs")
...
fun getInputsForTask(
  ...
  return try {
    ...
    val combinedDependsOn: Set<Task> = getCombinedDependsOn(task)
...
fun getCombinedDependsOn(task: Task): Set<Task> {
...
fun getDependsOnForTask(
  ...
  return try {
    val combinedDependsOn = getCombinedDependsOn(task)
