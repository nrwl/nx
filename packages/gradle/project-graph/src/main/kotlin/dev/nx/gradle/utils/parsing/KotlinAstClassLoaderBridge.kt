...
fun parseKotlinFileWithReflection(
  ...
  return try {
    val parserClass = classLoader.loadClass("dev.nx.gradle.utils.parsing.KotlinAstTestParserKt")
    val method =
        parserClass.getMethod("parseKotlinFileViaAst", File::class.java, Logger::class.java)

    @Suppress("UNCHECKED_CAST")
    val result = method.invoke(null, file, logger) as? Map<String, String>
    result?.toMutableMap()
  } catch (e: Exception) {
    logger?.info(
...
private fun createKotlinAstClassLoader(project: Project, logger: Logger?): ClassLoader? {
  ...
  if (kotlinVersion == null) {
    logger?.info("Cannot detect Kotlin version for project ${project.name}, skipping AST parsing")
    return null
  }
  ...
  return try {
    ...
    logger?.warn(
        "Failed to create Kotlin AST classloader: ${e.javaClass.simpleName} - ${e.message}")
    null
  }
}
