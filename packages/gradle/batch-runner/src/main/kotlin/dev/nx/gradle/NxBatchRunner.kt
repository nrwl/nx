...
fun runTasksInParallel(
        ...
        when (event) {
          ...
          is TaskFinishEvent -> {
            ...
            if (taskNames.contains(taskPath)) {
              ...
              val endTime = maxOf(System.currentTimeMillis(), result.endTime)
  ...
  taskResults.forEach { (taskName, result) ->
    result.terminalOutput = perTaskOutput[taskName] ?: globalOutput
  }
...
fun splitOutputPerTask(globalOutput: String): Map<String, String> {
  ...
  val taskHeaderRegex = Regex("(?=> Task (:[^\\s]+))") // Lookahead to keep the task line
