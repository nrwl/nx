...
fun addTestCiTargets(
    ...
    testFiles.filter { testFile ->
        ...
        val regex = "^(?!abstract).*?(Test)(s)?\\d*".toRegex(RegexOption.IGNORE_CASE) // find the files that match Test(any number) regex
    ...
    if (ciDependsOn.isNotEmpty()) {
        if (targets["ci"] != null) {
            val ciTarget = targets["ci"] as? MutableMap<String, Any?> ?: mutableMapOf<String, Any?>().also { targets["ci"] = it }
