...
describe('migrate-create-nodes-v2-to-create-nodes migration', () => {
  ...
  describe('rewriteCreateNodesV2Imports', () => {
    ...
    it('renames a value usage passed as a call argument', () => {
      const input =
        `import { createNodesV2 } from '@nx/react-native/plugin';\n` +
        `addPlugin(graph, '@nx/react-native/plugin', createNodesV2, {});\n`;
      expect(rewrite(input)).toBe(
        `import { createNodes } from '@nx/react-native/plugin';\n` +
          `addPlugin(graph, '@nx/react-native/plugin', createNodes, {});\n`
      );
    });

    it('renames value usages when deduped against an existing createNodes', () => {
      const input =
        `import { createNodes, createNodesV2 } from '@nx/react-native/plugin';\n` +
        `use(createNodesV2);\n`;
      expect(rewrite(input)).toBe(
        `import { createNodes } from '@nx/react-native/plugin';\n` +
          `use(createNodes);\n`
