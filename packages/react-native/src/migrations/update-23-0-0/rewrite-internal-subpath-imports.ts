...
const FROM_PREFIX = '@nx/react-native/src/';
const TO_PUBLIC = '@nx/react-native';
const TO_INTERNAL = '@nx/react-native/internal';

// Symbols exported from `@nx/react-native`'s public entry (packages/react-native/index.ts).
// A named import/export of one of these from `@nx/react-native/src/*` is routed to
// the public `@nx/react-native` entry; everything else goes to `@nx/react-native/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/react-native/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/react-native, internals to @nx/react-native/internal).`
    );
  }
 ...
 * that resolve to `@nx/react-native`'s public entry and the ones that don't. If both
