...
export default async function* reactNativeStorybookExecutor(
  ...
  logger.info(
    `${styleText('bold', styleText('cyan', 'info'))} To see your Storybook stories on the device, you should start your mobile app for the <platform> of your choice (typically ios or android).`
  );
...
export function runCliStorybook(
  ...
  if (storiesFiles.length === 0) {
    logger.warn(
      `${styleText('bold', styleText('yellow', 'warn'))} No stories found.`
    );
  }
