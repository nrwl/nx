...
const FROM_PREFIX = '@nx/esbuild/src/';
const TO_PUBLIC = '@nx/esbuild';
const TO_INTERNAL = '@nx/esbuild/internal';

// Symbols exported from `@nx/esbuild`'s public entry (packages/esbuild/index.ts).
// A named import/export of one of these from `@nx/esbuild/src/*` is routed to
// the public `@nx/esbuild` entry; everything else goes to `@nx/esbuild/internal`.
const PUBLIC_SYMBOLS: ReadonlySet<string> = new Set([
  'configurationGenerator',
  'esbuildInitGenerator',
  'nxVersion',
  'tsLibVersion',
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/esbuild/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/esbuild, internals to @nx/esbuild/internal).`
    );
  }
 ...
 * that resolve to `@nx/esbuild`'s public entry and the ones that don't. If both
