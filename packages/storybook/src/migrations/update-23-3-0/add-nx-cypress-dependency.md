...
Workspaces that already declare `@nx/cypress`, or reference it in none of these places, are left unchanged. If a file that mentions `@nx/cypress` or `@nx/storybook/presets/cypress` cannot be parsed and no reference is found elsewhere, the migration lists the file in its next steps so you can check it and add `@nx/cypress` yourself. This happens when the file has syntax errors, is nested too deeply to parse, or TypeScript cannot be loaded.
