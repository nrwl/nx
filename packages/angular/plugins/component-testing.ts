...
function normalizeBuildTargetOptions(
   ...
   if (offset) {
      ...
      const handlePolyfillPath = (polyfill: string) => {
         const maybeFullPath = join(
            ctContext.root,
            polyfill.split('/').join(sep)
         );
      ...
      buildOptions.fileReplacements = buildOptions.fileReplacements.map(
         (fr) => {
      ...
      if (offset) {
         ...
         if (buildOptions.stylePreprocessorOptions?.includePaths.length > 0) {
            buildOptions.stylePreprocessorOptions = {
               includePaths:
                  buildOptions.stylePreprocessorOptions.includePaths.map(
                     (path) => {
