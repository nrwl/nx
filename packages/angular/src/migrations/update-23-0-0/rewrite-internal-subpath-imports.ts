...
function matchesDropped(text: string): boolean {
...
const PUBLIC_SYMBOLS: ReadonlySet<string> = new Set<string>([]);
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/angular/src/* imports in ${touchedCount} file(s) ` +
