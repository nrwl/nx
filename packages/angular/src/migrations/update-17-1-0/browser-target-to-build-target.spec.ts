...
describe('browser-target-to-build-target migration', () => {
   ...
   it.each(executors)(
      ...
      async (executor) => {
         ...
         ).toBe('app1:serve:development');
         expect(
            project.targets.serve.configurations.production.browserTarget
         ).toBeUndefined();
         expect(
            project.targets.serve.configurations.production.buildTarget
         ).toBe('app1:serve:production');
      }
   );
      ...
      async (executor) => {
         updateJson<NxJsonConfiguration>(tree, 'nx.json', (json) => {
            ...
            json.targetDefaults.serve = {
               ...
               configurations: {
                  development: {
                     browserTarget: '{projectName}:serve:development',
                  },
                  production: {
                     browserTarget: '{projectName}:serve:production',
                  },
               },
            };
            return json;
         ...
         expect(
            nxJson.targetDefaults.serve.options.browserTarget
         ).toBeUndefined();
