   ...
   return getSourceNodes(source)
      ...
      .filter((expr) => {
         if (expr.expression.kind == tsModule.SyntaxKind.Identifier) {
            ...
            return (
               id === identifier && angularImports[`${moduleId}.`] === module
            );
         }
...
export function addImportToTestBed(
   ...
   const configureTestingModuleObjectLiterals = allCalls
      .filter(
         (c) =>
            c.expression.kind === tsModule.SyntaxKind.PropertyAccessExpression
      )
      .filter(
         (c: any) =>
...
export function getBootstrapComponent(
   ...
   if (!bootstrap) {
      throw new Error(
         `Cannot find bootstrap components in '${moduleClassName}'`
      );
   }
