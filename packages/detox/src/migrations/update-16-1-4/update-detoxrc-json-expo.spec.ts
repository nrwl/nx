...
describe('Update detoxrc for expo projects', () => {
   ...
   it(`should update .detoxrc.json`, async () => {
      ...
      expect(detoxrcJson).toEqual({
         apps: {
            'ios.eas': {
               build: 'npx nx run products:download --platform ios --distribution simulator --output=../../apps/products/dist/',
            },
            'android.eas': {
               build: 'npx nx run products:download --platform android --distribution simulator --output=../../apps/products/dist/',
               type: 'android.apk',
            },
            'ios.local': {
               build: 'npx nx run products:build --platform ios --profile preview --wait --local --no-interactive --output=../../apps/products/dist/Products.tar.gz',
            },
            'android.local': {
               build: 'npx nx run products:build --platform android --profile preview --wait --local --no-interactive --output=../../apps/products/dist/Products.apk',
               type: 'android.apk',
            },
         },
      });
