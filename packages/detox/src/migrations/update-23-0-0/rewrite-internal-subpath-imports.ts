...
const FROM_PREFIX = '@nx/detox/src/';
const TO_PUBLIC = '@nx/detox';
const TO_INTERNAL = '@nx/detox/internal';

// Symbols exported from `@nx/detox`'s public entry (packages/detox/index.ts).
// A named import/export of one of these from `@nx/detox/src/*` is routed to
// the public `@nx/detox` entry; everything else goes to `@nx/detox/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/detox/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/detox, internals to @nx/detox/internal).`
    );
  }
 ...
 * that resolve to `@nx/detox`'s public entry and the ones that don't. If both
