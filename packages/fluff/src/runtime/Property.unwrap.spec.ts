...
import { resetTestPropertyUnwrapReceivedValue, TestPropertyUnwrapChildComponent, testPropertyUnwrapReceivedValue } from './tests/TestPropertyUnwrapChildComponent.js';
...
describe('Property unwrapping in bindings', () =>
{
    ...
    {
        FluffBase.__e = [
            (t: TestPropertyUnwrapParentComponent): Property<number> => t.hostClass.childProp
        ];
        ...
        const childTag = 'test-prop-unwrap-child-' + Math.random().toString(36).slice(2);
        const parentTag = 'test-prop-unwrap-parent-' + Math.random().toString(36).slice(2);
        ...
        expect(testPropertyUnwrapReceivedValue).toBe(42);
        expect(testPropertyUnwrapReceivedValue).not.toBeInstanceOf(Property);
