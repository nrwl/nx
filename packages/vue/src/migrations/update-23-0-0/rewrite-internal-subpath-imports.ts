...
const FROM_PREFIX = '@nx/vue/src/';
const TO_PUBLIC = '@nx/vue';
const TO_INTERNAL = '@nx/vue/internal';

// Symbols exported from `@nx/vue`'s public entry (packages/vue/index.ts).
// A named import/export of one of these from `@nx/vue/src/*` is routed to
// the public `@nx/vue` entry; everything else goes to `@nx/vue/internal`.
...
export default async function rewriteInternalSubpathImports(
  ...
  if (touchedCount > 0) {
    logger.info(
      `Rewrote @nx/vue/src/* imports in ${touchedCount} file(s) ` +
        `(public symbols to @nx/vue, internals to @nx/vue/internal).`
    );
  }
 ...
 * that resolve to `@nx/vue`'s public entry and the ones that don't. If both
