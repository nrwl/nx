...
import { ensureDirSync, removeSync, moveSync, copySync } from 'fs-extra';
...
function patchPackageJsonForPlugin(npmPackageName: string, distPath: string) {
  const p = JSON.parse(readFileSync(tmpProjPath('package.json')).toString());
  p.devDependencies[npmPackageName] = `file:${distPath}`;
  writeFileSync(tmpProjPath('package.json'), JSON.stringify(p, null, 2));
}
...
export function runNew(
  ...
  let gen = runCliNewCommand(args, silent);
...
/**
 * Run the `new` command for the currently selected CLI for use with a plugin
 * @param npmPackageName The name of the package scope that the plugin should use
 * @param pluginDistPath The output directory of the plugin build
 * @param args Extra arguments to pass to the `new` command
 * @param silent Run in silent mode (no output)
 */
export function runNewForPlugin(
...
function runCliNewCommand(args: string, silent: boolean) {
  let gen;
  if (cli === 'angular') {
    gen = execSync(
      `npx ng new proj --no-interactive --skip-install ${args || ''}`,
      {
        cwd: `./tmp/${cli}`,
        ...(silent ? { stdio: ['ignore', 'ignore', 'ignore'] } : {})
      }
    ...
    gen = execSync(
      `npx tao new proj --no-interactive --skip-install ${args || ''}`,
      {
        cwd: `./tmp/${cli}`,
...
export function newProject(
  ...
  if (!directoryExists(tmpBackupProjPath())) {
    if (!npmPackageName) {
      ...
      runNewForPlugin(npmPackageName, pluginDistPath);
    }
...
export function ensureProject(
  ...
  newProject(npmPackageName, pluginDistPath);
}
