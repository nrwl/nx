               ...
               fix(fixer) {
                  if (targetProject) {
                     ...
                     if (indexTsPaths && indexTsPaths.length > 0) {
                        ...
                        const adjustedRelativeImports =
                           groupImports(importsToRemap);
                           ...
                           for (const importMember of imports) {
                              ...
                              if (importPath) {
                                 ...
                                 importsToRemap.push({
                                    ...
                                    importPath: importPathResolved.replace(
                                       /.tsx?$/,
                                       ''
                                    ),
                                 });
         ...
         if (depConstraints.length > 0) {
            ...
            for (let constraint of constraints) {
               ...
               if (
                  ...
                  if (matches.length > 0) {
                     matches.forEach(
                        ([target, violatingSource, constraint]) => {
                           context.report({
                              node,
