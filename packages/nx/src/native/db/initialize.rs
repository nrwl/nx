...
fn create_io_error(operation: &str, path: &Path, error: std::io::Error) -> anyhow::Error {
    ...
    let specific_guidance = match error.kind() {
        ErrorKind::PermissionDenied => {
            format!(
                "Possible solutions:\n\
                - Check file permissions: chmod u+w {}\n\
                - Ensure you own the file/directory: chown $USER {}\n\
    ...
    loop {
        match open_database_connection(db_path) {
            Ok(mut c) => {
                ...
                match db_version {
                    Ok(Some(version)) if version == nx_version => {
                        ...
                        // Try to configure - this is where journal mode is set
                        match configure_database(&c, db_path, attempt_number) {
                            Ok(_) => return Ok(c),
                            Err(config_error) if attempt_number < MAX_INIT_ATTEMPTS => {
                    ...
                    reason => {
                        ...
                        // Don't decrement retry attempts, but do increment attempt number for journal mode retry
                        attempt_number += 1;
                        continue;
                    }
                }
            ...
            Err(reason) => {
                ...
                if attempt_number < MAX_INIT_ATTEMPTS {
                    trace!("Removing existing incompatible database");
                    remove_all_database_files(db_path)?;
...
fn remove_all_database_files(db_path: &Path) -> anyhow::Result<()> {
    // SQLite appends -wal and -shm to the database filename
    ...
    let files_to_remove = [
        ...
        (wal_path, "WAL file"),
        (shm_path, "shared memory file"),
        // Note: lock file is managed separately via unlock_file() and should not be removed here
    ];
    ...
    for (path, description) in &files_to_remove {
        match remove_file(path) {
            ...
            Err(e) => {
                trace!("Warning: failed to remove {}: {}", description, e);
...
fn configure_database(
    ...
    let journal_mode = try_set_journal_mode(connection, db_path, attempt)?;
...
fn try_set_journal_mode(
    ...
    match connection.pragma_update(None, "journal_mode", "WAL") {
        ...
        Err(wal_error) => {
            trace!("WAL mode failed on attempt {}: {:?}", attempt, wal_error);

            // On first attempt, WAL failure might be due to stale auxiliary files
            // Return error to trigger cleanup and retry
            if attempt == 1 {
                trace!("WAL failed on first attempt - may be stale auxiliary files");
            ...
            // Second+ attempt failed - fall back to DELETE mode
        ...
        match write(&test_file, b"test") {
            ...
            Err(write_error) => {
                ...
                return anyhow::anyhow!(
                    ...
                    \n\
                    Possible solutions:\n\
                    - Check file permissions: chmod -R u+w {}\n\
                    - Ensure you own the directory: chown -R $USER {}\n\
                    - Verify the filesystem is not mounted read-only\n\
                    - If using Docker, ensure volume has correct permissions\n\
                    \n\
                    Permission error: {}\n\
