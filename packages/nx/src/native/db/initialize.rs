...
mod tests {
    ...
    #[test]
    fn failed_transaction_rolls_back_and_leaves_connection_usable() -> anyhow::Result<()> {
        let temp_dir = tempfile::tempdir()?;
        let conn = initialize_db(&temp_dir.path().join("test.db"))?;

        let result: anyhow::Result<()> = conn.transaction(|c| {
            c.execute("INSERT INTO metadata (key, value) VALUES ('k', 'v')", &[])?;
            anyhow::bail!("fail mid-transaction")
        });
        assert!(result.is_err());
        assert!(
            conn.query_row("SELECT 1 FROM metadata WHERE key='k'", &[])?
                .is_none()
        );

        conn.transaction(|c| {
            c.execute("INSERT INTO metadata (key, value) VALUES ('k', 'v2')", &[])?;
            Ok(())
        })?;
