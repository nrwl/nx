...
impl TasksList {
    ...
    fn get_header_cells(&self, has_narrow_area_width: bool) -> Vec<Cell> {
        let status_style = if !self.is_focused {
...
impl Component for TasksList {
    ...
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> Result<()> {
        ...
        let chunks = Layout::default()
            ...
            .constraints(if has_short_area_height {
                vec![
        ...
        let needs_vertical_bottom_layout = has_narrow_area_width || has_short_area_height;
        ...
        let help_text = HelpText::new(self.cloud_message.is_some(), !self.is_focused, false); // Use collapsed mode when cloud message is present
        ...
        if needs_vertical_bottom_layout {
            ...
            pagination.render(f, pagination_area, !self.is_focused);
            ...
            if let Some(message) = &self.cloud_message {
                ...
                if should_show_message {
                    ...
                    let message_line = if let Some(url_pos) = message.find("https://") {
                        ...
                        let url_style = if !self.is_focused {
                        ...
                        let message_style = if !self.is_focused {
                            Style::default().fg(Color::DarkGray).dim()
                        } else {
                            Style::default().fg(Color::DarkGray)
                        };
