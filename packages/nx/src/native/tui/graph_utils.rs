...
pub fn get_failed_dependencies(
    ...
    all_dependencies
        ...
        .filter(|dep| {
            matches!(
                status_map.get(dep).unwrap_or(&TaskStatus::NotStarted),
                TaskStatus::Failure
            )
        })
        .collect()
...
/// Gets all dependencies that have been skipped for a given task.
/// Returns a vector of task names that are dependencies and have been skipped.
pub fn get_skipped_dependencies(
    task_id: &str,
    task_graph: &TaskGraph,
    status_map: &HashMap<String, TaskStatus>,
) -> Vec<String> {
