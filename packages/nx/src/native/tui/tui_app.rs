...
use super::lifecycle::{
    BatchInfo, BatchStatus, CloudConnection, PerformanceSummaryPayload, TuiMode,
};
...
pub trait TuiApp: Send {
    ...
    fn set_cloud_connection(&mut self, status: CloudConnection) {
        self.state().lock().set_cloud_connection(status);
    }
    ...
    fn set_connect_url(&mut self, url: String) {
        self.state()
            .lock()
            .set_connect_flow_state(ConnectFlowState::Ready(url));
    }
    ...
    fn set_connect_error(&mut self, message: String) {
        self.state()
            .lock()
            .set_connect_flow_state(ConnectFlowState::Error(message));
    }
