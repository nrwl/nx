...
"#
    ...
    if rest.is_empty() {
        anyhow::bail!(
            r#""{s}" is an invalid fileset.
A root token on its own names a directory, which matches no files.
To include everything under it, use "{negation}{token}/**/*".
"#
        );
    }
...
mod tests {
    ...
    fn filesets_start_with_a_whole_root_token_segment_and_stay_under_it() {
        ...
        for (bad, reason) in [
            ("{projectRoot}", "use \"{projectRoot}/**/*\""),
            ("!{projectRoot}", "use \"!{projectRoot}/**/*\""),
            ("{workspaceRoot}", "use \"{workspaceRoot}/**/*\""),
