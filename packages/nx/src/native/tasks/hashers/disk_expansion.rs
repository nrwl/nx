...
/// Whether a configured skipped directory is a plain workspace-relative path.
/// An empty entry, `.`, `..` or an absolute path would name the root or leave
/// it, so it is not a skip.
...
/// The configured skipped directories as absolute paths under
/// `workspace_root`, see `is_skippable_dir`.
pub(crate) fn skip_dirs_under(workspace_root: &Path, dirs: &[String]) -> Vec<PathBuf> {
    dirs.iter()
...
pub(crate) mod tests {
    ...
    #[test]
    fn a_skipped_directory_must_be_a_plain_relative_path() {
        for dir in ["", ".", "./", "..", "../x", "a/../b", "/abs"] {
            assert!(!is_skippable_dir(dir), "{dir:?}");
        }
        for dir in ["tmp/nx-cache", "..cache", "tmp/nx-cache/"] {
            assert!(is_skippable_dir(dir), "{dir:?}");
        }
    }
