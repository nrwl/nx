    ...
    for (consumer, plan) in &hash_plans.plans {
        ...
        for id in plan {
            match hash_plans.pool.get(*id).value() {
                HashInstruction::ProjectFileSet(_, globs, true) => consumer_globs.extend(globs.iter().cloned()),
...
mod tests {
    ...
    fn negated_patterns_do_not_establish_an_edge() {
        ...
        let p = plans(vec![(
            "app:build",
            vec![HashInstruction::ProjectFileSet("app".into(), strings(&["!dist/libs/ui/**/*.map"]), true)],
        )]);
        assert!(edges_of(&p, &g).is_empty());
