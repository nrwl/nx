    ...
    ids.into_par_iter()
        .filter_map(|id| {
            let reads = match hash_plans.pool.get(id).value() {
                HashInstruction::ProjectFileSet(_, globs, true) => InstructionReads::Globs(positive_prefixes(globs)),
...
mod tests {
    ...
    fn reading(globs: &[&str]) -> Vec<HashInstruction> {
        vec![HashInstruction::ProjectFileSet("app".into(), strings(globs), true)]
    }
