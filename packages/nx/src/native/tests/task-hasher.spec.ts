...
describe('task hash result conversion and assembly', () => {
  // Jest runs each test file in a vm context with its own `Object`, while
  // objects the native module creates carry the host realm's prototype.
  const hostObjectPrototype: typeof Object.prototype =
    runInThisContext('Object.prototype');
  let tempFs: TempFs;
  ...
  it.each([false, true])(
    ...
    (collect) => {
      ...
      for (const id of ids) {
        ...
        expect(Object.getPrototypeOf(result)).toBe(hostObjectPrototype);
        expect(Object.getPrototypeOf(result.details)).toBe(hostObjectPrototype);
  ...
  it('recovers after a setter throws during result conversion', () => {
    ...
    Object.defineProperty(hostObjectPrototype, key, {
      configurable: true,
      set() {
    ...
    try {
      ...
      delete hostObjectPrototype[key];
    }
