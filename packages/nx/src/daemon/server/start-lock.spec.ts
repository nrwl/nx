...
/** A pid that is really gone: a spawned process that has exited. */
...
describe('daemon start lock', () => {
  ...
  asUser(
    ...
    async () => {
      // A failing takeover has nothing to wait on but the loop's own sleep. The
      // deadline bounds a spinning loop too, so wall time cannot tell; CPU can.
  ...
  it('takes over a lock a live holder has held far past any real boot', async () => {
    // After a reboot pids restart, so an unrelated live process can own the pid
    // in the file; without an upper bound it would never be removed.
...
describe('findHealthyDaemonOwner', () => {
  ...
  /** Same nx version, spelled out: an absent field would put every test on the mismatch branch. */
  ...
  it('probes the socket named by the registration, not one derived here', async () => {
    // The socket directory hashes its creator's pid, so a derived path names
    // nothing; the live socket under a different path separates the two.
    await listen();
  ...
  it('claims the workspace from a live answering daemon on another nx version', async () => {
    // The client never kills an incumbent on a version mismatch, so treating a
    // responsive old daemon as the owner strands its poll until the budget runs out.
    await listen();
    h.registration.value = registered({
      processId: process.ppid,
      socketPath,
