...
export async function handleCachePut(
  ...
  const expandedOutputs = getCacheInstance().put(
    hash,
    terminalOutput ?? '',
    outputs,
    code
  ...
  return {
    response: JSON.stringify(expandedOutputs),
...
export async function handleCacheGetSize() {
  return {
    response: JSON.stringify({ size: getCacheInstance().getCacheSize() }),
...
export async function handleCacheCheckFsInSync() {
  return {
    response: JSON.stringify({
      inSync: getCacheInstance().checkCacheFsInSync(),
    }),
