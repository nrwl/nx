...
describe('URL shorten various functions', () => {
   describe('compareCleanCloudVersions', () => {
      it('should return 1 if the first version is newer', () => {
         ...
         expect(compareCleanCloudVersions('2312.25.120', '2312.24.110')).toBe(
            1
         );
      });

      it('should return -1 if the first version is older', () => {
         expect(compareCleanCloudVersions('2312.25.50', '2407.01.100')).toBe(
            -1
         );
         expect(compareCleanCloudVersions('2312.31.100', '2401.01.100')).toBe(
            -1
         );
         expect(compareCleanCloudVersions('2312.25.100', '2312.26.50')).toBe(
            -1
         );
         expect(compareCleanCloudVersions('2311.25.90', '2312.25.80')).toBe(-1);
         expect(compareCleanCloudVersions('2312.24.110', '2312.25.100')).toBe(
            -1
         );
      });
      ...
      it('should return 0 if both versions are the same', () => {
         ...
         expect(compareCleanCloudVersions('2407.01.100', '2407.01.100')).toBe(
            0
         );
      });
   });
