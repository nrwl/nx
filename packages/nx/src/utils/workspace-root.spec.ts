...
const rootMarkers = ['nx.json', 'nx.sh', 'nx.bat']
...
describe('workspaceRootInner', () => {
  ...
  it.each(rootMarkers)('should find workspace root from %s when in subpackage', (marker) => {
    jest
      .spyOn(fileUtils, 'fileExists')
      .mockImplementation((p) =>
        [
  ...
  it.each(rootMarkers)('should prefer workspace root from %s when in subpackage containing nx', (marker) => {
    jest
      .spyOn(fileUtils, 'fileExists')
      .mockImplementation((p) =>
        [
