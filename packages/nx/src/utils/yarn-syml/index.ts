...
export function stringifySyml(value: any): string {
  try {
...
function parseViaPeg(source: string): { [key: string]: any } {
  if (!source.endsWith(`\n`)) source += `\n`;
...
function parseViaJsYaml(source: string): { [key: string]: any } {
  ...
  if (typeof value !== `object`)
    throw new Error(
      `Expected an indexed object, got a ${typeof value} instead. Does your file follow Yaml's rules?`
    );
  if (Array.isArray(value))
    throw new Error(
...
export function parseSyml(source: string): { [key: string]: any } {
