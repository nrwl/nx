...
describe.only('cache-directory', () => {
  ...
  it('projectGraphCacheDirectory should default to what cacheDir is', () => {
  ...
  it('projectGraphCacheDirectory in nx.json should work with relative path', () => {
    const spec = projectGraphCacheDirSpec('not/a/directory/we/should/see');
    const nxJson = emptyNxJSon();
    nxJson.tasksRunnerOptions.default.options['projectGraphCacheDirectory'] =
      '.nxproject';
    const dir = cacheDirectory(root(), emptyEnv(), nxJson, spec);
    expect(dir).toBe(join(root(), '.nxproject'));
  });

  it('projectGraphCacheDirectory in nx.json should work with absolute path', () => {
    const spec = projectGraphCacheDirSpec('not/a/directory/we/should/see');
    const nxJson = emptyNxJSon();
    nxJson.tasksRunnerOptions.default.options['projectGraphCacheDirectory'] =
      '/tmp/.nxproject';
    const dir = cacheDirectory(root(), emptyEnv(), nxJson, spec);
    expect(dir).toBe(join('/tmp/.nxproject'));
  });
  ...
  it('env NX_PROJECT_GRAPH_CACHE_DIRECTORY should have priority over nx.json', () => {
    const spec = projectGraphCacheDirSpec('not/a/directory/we/should/see');
    const env = { NX_PROJECT_GRAPH_CACHE_DIRECTORY: 'nx-project-graph' };
    const nxJson = emptyNxJSon();
    nxJson.tasksRunnerOptions.default.options['projectGraphCacheDirectory'] =
