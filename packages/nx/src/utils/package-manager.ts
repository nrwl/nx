...
export function modifyYarnRcToFitNewDirectory(contents: string): string {
   ...
   const yarnPathIndex = lines.findIndex((line) =>
      line.startsWith('yarn-path')
   );
   if (yarnPathIndex !== -1) {
   ...
   for (const packageManagerConfigFile of [
      ...
      if (f) {
         ...
         switch (packageManagerConfigFile) {
            ...
            case '.yarnrc': {
               const updated = modifyYarnRcToFitNewDirectory(
...
export async function resolvePackageVersionUsingRegistry(
   ...
   try {
      ...
      if (!result) {
         throw new Error(
            `Unable to resolve version ${packageName}@${version}.`
         );
      }
