...
describe('semver', () => {
   ...
   describe('determineSemverChange()', () => {
      const config: NxReleaseConfig['conventionalCommits'] = {
         types: {
            feat: {
               semverBump: 'minor',
               changelog:
                  DEFAULT_CONVENTIONAL_COMMITS_CONFIG.types.feat.changelog,
            },
            fix: {
               semverBump: 'patch',
               changelog:
                  DEFAULT_CONVENTIONAL_COMMITS_CONFIG.types.fix.changelog,
            },
            chore: {
               semverBump: 'patch',
               changelog:
      ...
      it('should return null when given only unknown type commits', () => {
         expect(determineSemverChange([unknownTypeCommit], config)).toEqual(
            null
         );
      });
   });
