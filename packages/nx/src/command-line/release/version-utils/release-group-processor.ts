...
Valid values are: ${validReleaseVersionPrefixes
    ...
    // for (const [, groupNode] of this.groupGraph) {
  ...
  private async bumpIndependentVersionGroup(
    ...
    for (const project of releaseGroupFilteredProjects) {
      if (projectsToUpdate.has(project)) {
        ...
        verboseLog(
          `Second pass - Project: ${project}, Final bump type: ${finalBumpType}, Bump type reason: ${finalBumpTypeReason}, Bump type reason data: ${JSON.stringify(
    ...
    for (const [source, deps] of Object.entries(dependencies)) {
      ...
      for (const dep of deps) {
        if (dep && dep.target === project) {
          ...
          try {
            const manifestData =
              await manifestActions.readCachedSourceManifestData(this.tree);
