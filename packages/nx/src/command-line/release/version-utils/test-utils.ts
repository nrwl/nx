...
export class ExampleRustManifestActions extends ManifestActions {
  ...
  async readCurrentVersionFromManifest(tree: Tree): Promise<string> {
  ...
  async readCurrentVersionFromRegistry(
    ...
    return {
      currentVersion: await this.readCurrentVersionFromManifest(tree),
      ...
      if (dependsOn) {
        dependsOn.forEach(
          (dep: { project: string; collection: string; prefix: string }) => {
            ...
            const depVersion = depNode.version;
            packageJson[dep.collection][
