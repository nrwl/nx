...
async function releaseVersionGeneratorForTest(
  ...
  const { releaseVersionGenerator } = await import('../version');
  return releaseVersionGenerator(tree, {
    nxReleaseConfig,
    projectGraph,
    releaseGroups,
    releaseGroupToFilteredProjects,
...
// TODO: Port 3 remaining missing tests
describe('releaseVersionGenerator (ported tests)', () => {
    ...
    it(`should exit with code one and print guidance when not all of the given projects are appropriate for JS versioning`, async () => {
      ...
      expect(outputSpy).toHaveBeenCalledWith({
        title: expect.stringContaining(
          'The project "my-lib" does not have a manifest available at my-lib/package.json'
        ),
      });
  ...
  describe('updateDependentsOptions', () => {
    ...
    //   const { nxReleaseConfig, projectGraph, releaseGroups, releaseGroupToFilteredProjects } =
    //     await createNxReleaseConfigAndPopulateWorkspace(
    //       tree,
    //       `
    //         __default__ ({ "projectsRelationship": "independent" }):
