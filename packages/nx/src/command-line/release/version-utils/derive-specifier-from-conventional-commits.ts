...
export async function deriveSpecifierFromConventionalCommits(
  ...
  if (
    releaseGroup.version.generatorOptions.currentVersionResolver !== 'git-tag'
  ) {
    throw new Error(
      `Invalid currentVersionResolver "${releaseGroup.version.generatorOptions.currentVersionResolver}" provided for release group "${releaseGroup.name}". Must be "git-tag" when "specifierSource" is "conventional-commits"`
    );
  }
