...
export async function resolveCurrentVersion(
  ...
  let currentVersionResolverMetadata:
    ...
    | undefined;
  let fallbackCurrentVersionResolver:
    | ReleaseVersionGeneratorSchema['fallbackCurrentVersionResolver']
    | undefined;
...
export async function resolveCurrentVersionFromDisk(
  ...
  const currentVersion = await manifestActions.readCurrentVersionFromManifest(
    tree
  );
  ...
  try {
    ...
    if (fallbackCurrentVersionResolver === 'disk') {
      const currentVersionFromDisk =
        await manifestActions.readCurrentVersionFromManifest(tree);
      // Fallback on disk is available, return it directly
      if (currentVersionFromDisk) {
        logger.buffer(
          `⚠️  Unable to resolve the current version from the registry${registryTxt}. Falling back to the version ${currentVersionFromDisk} in manifest: ${manifestActions.getPrimaryManifestPath()}`
        );
        // Write to the cache if the release group is fixed
        if (releaseGroup.projectsRelationship === 'fixed') {
          cachedCurrentVersionsPerFixedReleaseGroup.set(releaseGroup.name, {
...
export async function resolveCurrentVersionFromGitTag(
  ...
  if (fallbackCurrentVersionResolver === 'disk') {
    ...
    if (currentVersionFromDisk) {
      logger.buffer(
        `⚠️  Unable to resolve the current version from git tags using pattern "${releaseTagPattern}". Falling back to the version ${currentVersionFromDisk} in manifest: ${manifestActions.getPrimaryManifestPath()}`
      );
      // Write to the cache if the release group is fixed
      if (releaseGroup.projectsRelationship === 'fixed') {
        cachedCurrentVersionsPerFixedReleaseGroup.set(releaseGroup.name, {
...
async function handleNoAvailableDiskFallback({
  ...
  const unresolvableCurrentVersionError = new Error(
    `Unable to resolve the current version ${currentVersionSourceMessage} and there is no version on disk to fall back to. This is invalid with ${specifierSource} because the new version is determined by relatively bumping the current version. To resolve this, ${resolutionSuggestion}, or set an appropriate value for "version" in ${manifestPath}`
  );
