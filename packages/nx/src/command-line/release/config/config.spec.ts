...
describe('createNxReleaseConfig()', () => {
  ...
  describe('useLegacyVersioning', () => {
    ...
    it('should be respected if set to false by the user', async () => {
      const res = await createNxReleaseConfig(projectGraph, projectFileMap, {
        version: { useLegacyVersioning: false },
      });
      expect(res.nxReleaseConfig.version.useLegacyVersioning).toBe(false);
    });

    it('should set the versionActions and versionActionsOptions, and not set generator or generatorOptions, if set to false by the user', async () => {
      const res = await createNxReleaseConfig(projectGraph, projectFileMap, {
        version: { useLegacyVersioning: false },
      });
      ...
      expect((version as any).generatorOptions).toBeUndefined();
    });

    it('should respect custom versionActions and versionActionsOptions if set by the user', async () => {
      const res = await createNxReleaseConfig(projectGraph, projectFileMap, {
        version: {
          useLegacyVersioning: false,
          versionActions: '/path/to/custom-version-actions',
