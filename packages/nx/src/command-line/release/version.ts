...
export function createAPI(overrideReleaseConfig: NxReleaseConfiguration) {
  ...
  return async function releaseVersion(
    ...
    const { callback, data } = await releaseVersionGenerator(tree, {
      nxReleaseConfig,
      projectGraph,
      releaseGroups,
...
async function runVersionOnProjects(
  ...
  const generatorOptions: ReleaseVersionGeneratorSchema = {
    ...
    // TODO: consider removing this in favour of the dynamic release API client
    ...(generatorOverrides ?? {}),
    // The properties below this line are not overridable via generatorOptions, but are necessary for the generator to function appropriately
