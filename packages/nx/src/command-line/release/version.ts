...
export function createAPI(overrideReleaseConfig: NxReleaseConfiguration) {
  ...
  return async function releaseVersion(
    ...
    // TODO: Remove support for the legacy versioning implementation in Nx v22
    if (
      // Allow internal env var override to make e2e tests easier to run on both legacy and new versioning
      process.env.NX_INTERNAL_USE_LEGACY_VERSIONING !== 'false' &&
      nxReleaseConfig.version.useLegacyVersioning
    ) {
