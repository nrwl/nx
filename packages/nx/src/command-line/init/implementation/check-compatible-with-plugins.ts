...
export async function checkCompatibleWithPlugins(
  ...
  try {
    const plugins: LoadedNxPlugin[] = await getPlugins();
    await retrieveProjectConfigurations(plugins, root, readNxJson(root));
  } catch (projectConfigurationsError) {
    if (isProjectConfigurationsError(projectConfigurationsError)) {
