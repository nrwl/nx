...
function categorizeInputs(
  ...
  inputs.forEach((input) => {
    ...
    if (input.startsWith('{projectRoot}')) {
      ...
      projectRootInputs.push(`${projectName}:${input}`);
      return;
    }
...
> {
  return projectRootInputs
    .map((plan) => {
      const fileSetProjectName = plan.split(':')[0];
      const fileSetProject = depGraphClientResponse.projects.find(
        (p) => p.name === fileSetProjectName
      );
      const fileSets = plan.replace(`${fileSetProjectName}:`, '').split(',');

      const matches: {
        [plan: string]: string[];
      } = {
        [plan]: filterUsingGlobPatterns(
