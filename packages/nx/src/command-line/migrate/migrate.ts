      ...
      for (const packageToCheck of packagesToCheck) {
         ...
         for (const packageUpdate of packageToCheck.updates) {
            if (
               ...
               Object.entries(packageUpdate.packages).forEach(
                  ([name, update]) => {
...
async function versionOverrides(overrides: string, param: string) {
   ...
   const promises = overrides.split(',').map((p) => {
      ...
      return normalizeVersionWithTagCheck(
         selectedPackage,
         selectedVersion
      ).then((version) => {
...
export async function parseMigrationsOptions(options: {
   ...
   if (!options.runMigrations) {
      ...
      const { targetPackage, targetVersion } =
         await parseTargetPackageAndVersion(options['packageAndVersion']);
      return {
         type: 'generateMigrations',
      ...
      try {
         ...
         if (!cache[packageName]) {
            ...
            if (!path.startsWith(workspaceRoot)) {
               throw new Error(
                  'Resolved a package outside the workspace root.'
               );
            }
...
async function getPackageMigrationsConfigFromRegistry(
   ...
   if (!json['nx-migrations'] && !json['ng-update']) {
      const registry = new URL(
         'dist' in json ? json.dist.tarball : json.tarball
      ).hostname;
   ...
   for (const m of sortedMigrations) {
      try {
         ...
         if (!isAngularMigration(collection, collectionPath, m.name)) {
            ...
            const { madeChanges, loggingQueue } =
               await ngCliAdapter.runMigration(
                  root,
                  m.package,
                  m.name,
                  readProjectsConfigurationFromProjectGraph(
