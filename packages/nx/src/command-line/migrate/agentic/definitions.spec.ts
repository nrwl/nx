...
function makeContext(
  ...
  return {
    ...
    systemPromptFilePath: '/workspace/.nx/migrate-runs/1.0.0/pkg/m.system.md',
    instructionsPointer: 'read .nx/migrate-runs/1.0.0/pkg/m.instructions.md',
  ...
  it('builds the interactive spec with pre-authorized handoff writes, --system-prompt-file, and the instructions pointer', () => {
    ...
    expect(spec).toEqual({
      args: [
        ...
        '--system-prompt-file',
        '/workspace/.nx/migrate-runs/1.0.0/pkg/m.system.md',
        'read .nx/migrate-runs/1.0.0/pkg/m.instructions.md',
      ],
      cwd: '/workspace',
    });
  });
...
describe('opencodeDefinition', () => {
  ...
  it('references the system prompt file via OPENCODE_CONFIG_CONTENT under the transient agent name', () => {
    ...
    expect(parsed).toEqual({
      agent: {
        'nx-migrate': {
          prompt: '{file:/workspace/.nx/migrate-runs/1.0.0/pkg/m.system.md}',
        },
      },
    });
  });
