...
describe('writeStepInstructionFiles', () => {
  ...
  const instructionsRelativePath =
    '.nx/migrate-runs/23.1.0/@nx/eslint/update-23-1-0.instructions.md';
  ...
  it('writes both prompts beside the step handoff file', () => {
    ...
    expect(files.systemPromptFilePath).toBe(
      join(runDir, '@nx', 'eslint', 'update-23-1-0.system.md')
    );
  ...
  // re-require under a win32 `relative`: a spy on the `path` namespace does not
  // reach the module's own import binding.
  it('rewrites Windows separators in the pointer to forward slashes', () => {
    jest.doMock('path', () => {
    ...
    try {
      jest.isolateModules(() => {
        const {
          writeStepInstructionFiles: writeWithWin32Relative,
        } = require('./instruction-files');
  ...
  it('sanitizes migration identifiers into the file names', () => {
    mkdirSync(join(runDir, '@scope', 'pkg'), { recursive: true });
    ...
    expect(files.systemPromptFilePath).toBe(
      join(runDir, '@scope', 'pkg', '_.system.md')
    );
  });
