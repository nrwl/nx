...
export function buildGenericValidationUserPrompt(
  ...
  lines.push(
    ...
    firstStep,
    `2. Pick the smallest relevant subset of available targets to verify the change. Prefer \`nx affected --files=<comma-separated changed paths> -t <target>\` with the changed file list (bare \`nx affected\` also selects the branch delta and unrelated uncommitted changes). \`--files\` splits on every comma, so when a changed path contains one, pipe the paths one per line into \`nx affected --stdin -t <target>\` instead (POSIX shells: \`printf '%s\\n' <paths> | nx affected --stdin -t <target>\`). Alternatively, use \`nx run <project>:<target>\` for a single project. When many small projects are affected, you may use \`nx run-many -t <target> -p <project1>,<project2>\` with the project list derived from the changed files. Unscoped \`nx run-many\` (no \`-p\`) is forbidden.`,
