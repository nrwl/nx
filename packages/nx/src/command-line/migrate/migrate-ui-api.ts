...
export function addSuccessfulMigration(
  ...
  return (
    ...
    // A rerun re-records the same id, so carry forward an ack the user made
    // but not one synthesized here from an earlier `skipAgentic`, which would
    // otherwise mark the prompt done on a rerun that now needs it. Waiving
    // sets the ack because nothing is owed.
