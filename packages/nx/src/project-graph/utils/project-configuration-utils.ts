   ...
   for (const [metadataKey, value] of Object.entries(metadata)) {
      ...
      if (Array.isArray(value) && Array.isArray(existingValue)) {
         for (const item of [...value]) {
            ...
            if (sourceMap) {
               sourceMap[
                  `${baseSourceMapPath}.${metadataKey}.${newLength - 1}`
               ] = sourceInformation;
            }
         }
         ...
         for (const key in value) {
            ...
            if (Array.isArray(value[key]) && Array.isArray(existingValue)) {
               for (const item of value[key]) {
                  ...
                  if (sourceMap) {
                     sourceMap[
                        `${baseSourceMapPath}.${metadataKey}.${key}.${i - 1}`
                     ] = sourceInformation;
                  }
               }
            } else {
               ...
               for (const k in value) {
                  ...
                  if (Array.isArray(value[k])) {
                     for (let i = 0; i < value[k].length; i++) {
                        sourceMap[
                           `${baseSourceMapPath}.${metadataKey}.${k}.${i}`
                        ] = sourceInformation;
                     }
                  }
               }
...
function mergeConfigurations<T extends Object>(
   ...
   if (projectConfigSourceMap) {
      for (const newConfiguration in newConfigurations) {
         ...
         for (const configurationProperty in newConfigurations[
            newConfiguration
         ]) {
            projectConfigSourceMap[
