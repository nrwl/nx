...
function normalizeOptions(
   ...
   options.unknownOptions = Object.keys(options)
      .filter(
         (p) =>
            propKeys.indexOf(p) === -1 && unparsedCommandArgs[p] === undefined
      )
...
function nodeProcess(
   ...
   return new Promise((res) => {
      ...
      childProcess.stdout.on('data', (data) => {
         ...
         if (
            readyWhenStatus.length &&
            isReady(readyWhenStatus, data.toString())
         ) {
      ...
      childProcess.stderr.on('data', (err) => {
         ...
         if (
            readyWhenStatus.length &&
            isReady(readyWhenStatus, err.toString())
         ) {
            res({ success: true, terminalOutput });
         }
      });
         ...
         if (element.includes('=')) {
            ...
            if (propKeys.includes(key)) {
               if (
                  ...
                  parseArgs[key] &&
                  __unparsed__[index + 1].toString() ===
                     parseArgs[key].toString()
               ) {
