         ...
         for (const [alias, { version }] of Object.entries(
            ...
            if (oldIdToNewId[version]) {
               if (oldIdToNewId[version].startsWith(`${alias}@`)) {
                  importer[depType]![alias].version = oldIdToNewId[
                     version
...
function normalizeLockfileV6(lockfile: Lockfile, isLockfileV6: boolean) {
   ...
   if (
      Object.keys(lockfile.importers).length === 1 &&
      lockfile.importers['.']
   ) {
      lockfileToSave = {
         ...lockfile,
         ...
         importers: mapValues(
            ...
            (importer: ProjectSnapshot) => {
               ...
               if (
                  ...
                  normalizedImporter['dependenciesMeta'] =
               ...
               if (importer.publishDirectory) {
                  normalizedImporter.publishDirectory =
...
function dpParse(dependencyPath) {
   ...
   if (!_isAbsolute)
      throw new Error(
         `${dependencyPath} is an invalid relative dependency path`
      );
   return {
      host,
      isAbsolute: _isAbsolute,
