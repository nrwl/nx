...
/**
 * Reports `workspace:<name>@<range>` aliases whose target is absent from the
 * package-manager workspace names. Failing during graph construction
 * surfaces the broken manifest early with a repair hint.
 *
...
function buildInvalidWorkspaceAliasMessage(
  ...
  // Convert workspace: to npm: so registry fallback retains the alias target.
  lines.push(
