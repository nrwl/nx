...
const nx_commands_js_1 = require("../src/command-line/nx-commands.js");
const strip_indents_js_1 = require("../src/utils/strip-indents.js");
const nx_console_prompt_js_1 = require("../src/utils/nx-console-prompt.js");
/**
...
async function initLocal(workspace) {
    ...
    try {
        ...
        if (workspace.type !== 'nx' && shouldDelegateToAngularCLI()) {
            console.warn((0, strip_indents_js_1.stripIndents) `Using Nx to run Angular CLI commands is deprecated and will be removed in a future version.
        ...
        try {
            await (0, nx_console_prompt_js_1.ensureNxConsoleInstalled)();
        }
        catch { }
        const command = process.argv[2];
        if (command === 'run' || command === 'g' || command === 'generate') {
            nx_commands_js_1.commandsObject.parse(process.argv.slice(2));
        }
            ...
            if (help > -1 && (split === -1 || split > help)) {
                nx_commands_js_1.commandsObject.showHelp();
            }
            else {
                nx_commands_js_1.commandsObject.parse(newArgs);
            }
        }
        else {
            nx_commands_js_1.commandsObject.parse(process.argv.slice(2));
...
function isKnownCommand(command) {
    const commands = [
        ...Object.keys(nx_commands_js_1.commandsObject
            .getInternalMethods()
            .getCommandInstance()
            .getCommandHandlers()),
        'g',
