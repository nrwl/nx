...
const defaultChangelogRenderer: ChangelogRenderer = async ({
   ...
   if (project === null) {
      ...
      for (const type of Object.keys(changeTypes)) {
         ...
         const changesGroupedByScope: Record<string, ChangelogChange[]> =
            groupBy(changesInChronologicalOrder, 'scope');
            ...
            for (const change of changes) {
               ...
               if (change.isBreaking) {
                  const breakingChangeExplanation =
                     extractBreakingChangeExplanation(change.body);
   ...
   if (changelogRenderOptions.authors) {
      const _authors = new Map<
         string,
         { email: Set<string>; github?: string }
      >();
      for (const change of changes) {
         if (!change.author) {
            continue;
