...
export async function syncGenerator(tree: Tree, options: SyncSchema) {
   ...
   let tscPluginConfig:
      | { plugin: string; options?: TscPluginOptions }
      | string = nxJson.plugins.find((p) => {
      if (typeof p === 'string') {
   ...
   for (const [name, data] of firstPartyDeps) {
      ...
      const sourceTsconfig = readJson<Tsconfig>(
         tree,
      ...
      for (const dep of data) {
         ...
         if (
            ...
            sourceTsconfig.references.unshift({
               path: relativePathToTargetRoot,
            });
         }
      }
