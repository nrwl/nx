...
The migration appends the three artifact paths to the `outputs` of every target using the `@nx/js:prune-lockfile` executor whose `outputs` already contain a `pnpm-lock.yaml` entry, reusing that entry's own path prefix. Matching `targetDefaults` entries are updated the same way. Targets without `outputs`, targets whose lockfile entry is not pnpm's, and entries already present are left untouched.
