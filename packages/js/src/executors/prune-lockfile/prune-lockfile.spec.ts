...
describe('pruneLockfileExecutor - workspace module manifest rewrite', () => {
  ...
  it('rewrites an alias even after the real npm stringifier processed the manifest', async () => {
    ...
    // Compose the real prune/stringify path with an inline root lockfile;
    // stringification must preserve aliases for the executor's later rewrite.
    (
      jest.requireMock('nx/src/plugins/js/lock-file/lock-file')
        .generatePrunedDeployOutput as jest.Mock
    ).mockImplementationOnce((pkgJson, graph, _projectRoot, opts) => {
      writeFileSync(
        join(opts.outputDirectory, 'package-lock.json'),
