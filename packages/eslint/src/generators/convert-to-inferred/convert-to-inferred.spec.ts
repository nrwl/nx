   ...
   describe('--all', () => {
      it('should successfully migrate a project using Eslint executors to plugin', async () => {
         ...
         if (typeof hasEslintPlugin !== 'string') {
            [['targetName', 'lint']].forEach(
               ([targetOptionName, targetName]) => {
                  expect(hasEslintPlugin.options[targetOptionName]).toEqual(
                     targetName
                  );
               }
            );
