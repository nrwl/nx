...
export function removeOverridesFromLintConfig(content: string): string {
   ...
   exportsArray.forEach((node, i) => {
      if (isOverride(node)) {
         const commaOffset =
            i < exportsArray.length - 1 || exportsArray.hasTrailingComma
               ? 1
               : 0;
         changes.push({
      ...
      if (isOverride(node)) {
         ...
         if (ts.isObjectLiteralExpression(node)) {
            ...
            start =
               ...
               node['expression'].arguments[0].body.expression.properties.end;
         }
                  ...
                  if (plugins.length > 1) {
                     ...
                     if (keys.length > 1) {
                        ...
                        changes.push({
                           ...
                           length:
                              element.end - element.pos + (removeComma ? 1 : 0),
                        });
                     }
                  }
               } else if (
                  ts.isObjectLiteralExpression(pluginsElem.initializer)
               ) {
...
function addTSObjectProperty(
   ...
   if (value) {
      elements.push(
         ts.factory.createPropertyAssignment(key, generateAst(value))
      );
   }
}
