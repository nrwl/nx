...
Both the Starlight top banner and floating banner are configured via `banner.middleware.ts` and share the same config.

### Updating the Banner

**Preferred:** Set `BANNER_URL` in Netlify to a hosted JSON file. This allows updates without code changes.

**Temporary:** Until the env var is configured in Netlify, edit `src/banner-config.json` directly:
...
- **enabled**: Set `false` to hide both banners.
