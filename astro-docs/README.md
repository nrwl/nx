...
Set the `BANNER_URL` environment variable to point to your banner JSON:

```bash
BANNER_URL=https://example.com/banner.json nx serve astro-docs
```
...
1. Edit the JSON file at the URL specified in `BANNER_URL`
...
> **Note**: This is separate from the Starlight top banner configured in `src/content/notifications.json`, which is for site-wide announcements within the docs navigation.
