...
When developing the Nx sync generator, disable the [Nx Daemon](/docs/concepts/nx-daemon) by setting `NX_DAEMON=false`. The daemon caches your plugin code, so changes to your plugin won't be reflected until the daemon restarts.
{% /aside %}
...
```jsonc
// tools/my-plugin/src/generators/my-sync-generator/schema.json
{
...
```ts
// tools/my-plugin/src/generators/my-sync-generator/schema.d.ts
...
{% aside type="note" title="Important: Package.json Configuration" %}
For projects using [inferred targets](/docs/concepts/inferred-tasks) (no project.json file), the sync generators must be registered inside the `nx` property in package.json, not at the root level.
{% /aside %}
