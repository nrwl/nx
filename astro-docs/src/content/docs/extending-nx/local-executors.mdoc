...
[Node.js `childProcess`](https://nodejs.org/api/child_process.html) is often useful in executors.
...
For other ideas on how to create your own executors, you can always check out Nx own open-source executors as well!

{% github_repository url="https://github.com/nrwl/nx/blob/master/packages/cypress/src/executors/cypress/cypress.impl.ts" /%}
...
This would allow you to write a custom function in `hasher.ts`, which Nx would use to calculate the target's hash. As an example, consider the below hasher which mimics the behavior of Nx default hashing algorithm.
