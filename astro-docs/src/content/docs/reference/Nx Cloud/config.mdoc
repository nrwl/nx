---
title: 'Configuring the Cloud Runner'
description: 'Configuration options for Nx Cloud runner in nx.json'
...
If your organization has a security reason to never use Nx Cloud, you can cause all methods of setting up Nx Cloud to fail by adding the `neverConnectToCloud` property to `nx.json`.

This **does not prevent using Nx Cloud if already connected** nor disables the prompts themselves, as the `nx-cloud` package handles this property to provide maximum compatibility with Nx.

To prevent Nx Cloud from being used when running commands, set the `NX_NO_CLOUD=true` environment variable or use the `--no-cloud` arg instead.
