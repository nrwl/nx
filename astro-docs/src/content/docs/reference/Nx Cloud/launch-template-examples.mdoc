---
title: 'Launch Template Examples'
description: 'Common configuration examples for Nx Agents launch templates'
filter: 'type:References'
---

This page contains common configuration examples for [launch templates](/docs/reference/nx-cloud/launch-templates). For the full reference of available fields, see the [launch templates reference page](/docs/reference/nx-cloud/launch-templates).
...
```shell
nx start-ci-run --distribute-on="4 my-linux-medium-js"
```

```shell
nx start-ci-run --distribute-on="4 my-linux-large-js"
```

```shell
nx start-ci-run --distribute-on="4 my-linux-large-rust"
```

## Pass environment variables

If you need to send environment variables to agents, you can use the [--with-env-vars](/docs/reference/nx-cloud-cli#withenvvars-nx-agents-only) flag on the `nx start-ci-run` command. You can pass a specific list of environment variables like this:

```shell
nx start-ci-run --distribute-on="8 linux-medium-js" --with-env-vars="VAR1,VAR2"
```
...
nx start-ci-run --distribute-on="5 my-linux-medium-js" --with-env-vars="SOME_AUTH_TOKEN"
```
...
You can use [mise](#install-tools-with-mise) to manage node versions alongside other toolchains.
...
Nx Cloud provides a [pre-built step to install a custom node version](https://github.com/nrwl/nx-cloud-workflows/tree/main/workflow-steps/install-node) within your workflow. This step is available as of `v4` of the workflow steps and requires the minimum image version to be `ubuntu22.04-node20.11-v9` or later.
...
[mise](https://mise.jdx.dev/) is a polyglot version manager that can install and manage multiple tools and runtimes (node, Rust, Python, Go, and more) in a single step. Nx Cloud provides a [pre-built step](https://github.com/nrwl/nx-cloud-workflows/tree/main/workflow-steps/install-mise) to install mise and configure tools on your agents.

{% tabs %}
{% tabitem label="With mise.toml" %}
...
{% /tabitem %}
{% tabitem label="With inline tools" %}

You can define tools inline using the `tools` input. Each line specifies a tool and version separated by `=`, `@`, or a space.
...
{% aside type="caution" %}
Defining tools inline overrides any `mise.toml` in your repo. This can cause differences between CI and local environments. Use inline tools only when your repo does not have a `mise.toml`.
{% /aside %}
...
First, define the environment variables on your main agent, where you call [`nx start-ci-run`](/docs/reference/nx-cloud-cli#npx-nxcloud-startcirun). Pass the same environment variables to your agents via the [`--with-env-vars` flag](/docs/reference/nx-cloud-cli#withenvvars-nx-agents-only).
...
```shell
nx start-ci-run --distribute-on=".nx/workflows/dynamic-changesets.yaml"
```
