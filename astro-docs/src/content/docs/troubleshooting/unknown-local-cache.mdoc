...
When using the legacy file system cache ([deprecated in Nx 20](/docs/reference/deprecated/legacy-cache)), you can prefix any Nx command with `NX_REJECT_UNKNOWN_LOCAL_CACHE=0` to ignore the errors (
...
Storing Nx local cache on a network drive presents security risks, so we prevent you from doing so. When a network drive is shared, every CI run has access to all the previously created Nx cache artifacts. Hence, it is plausible for every single artifact - for every
...
[Nx Replay](/docs/features/ci-features/remote-cache), the Nx Cloud hosted remote cache, does the following things to make sharing the cache safe:
