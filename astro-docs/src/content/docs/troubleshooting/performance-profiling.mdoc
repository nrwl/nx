...
2. Open the Performance Tab in Chrome Devtools
   ![Performance Profiling with Chrome Devtools](../../../assets/troubleshooting/performance-profiling-devtools.png)
...
Nx offloads much of the heavy lifting to the [Nx Daemon](/docs/concepts/nx-daemon), which helps speed up task execution. However, you might still encounter slowdowns, not during the task execution itself, but in the moments leading up to it when invoking tasks through Nx. This can often happen when you're using a large number of [Nx Plugins](/docs/plugin-registry), whether they come from the core team or the community.

To investigate and optimize these slowdowns, you can use the `NX_PERF_LOGGING` [environment variable](/docs/reference/environment-variables). This will give you detailed performance logs and more precise timings, allowing you to pinpoint where the delays occur.
