...
{% aside type="note" title="Required Angular version" %}
...
{% aside type="note" title="Nx executors for Angular" %}
...
```ts {% meta="{7}" %}
// apps/my-app/src/app/api-http-client.ts
...
```js
// apps/my-app/plugins/env-var-plugin.js
...
{% aside type="note" title="Set environment variables from the terminal" %}
Alternatively, you can also [set environment variables when running a terminal command](/docs/guides/tips-n-tricks/define-environment-variables#adhoc-variables).
{% /aside %}
...
{% aside type="note" title="Optimize" %}
