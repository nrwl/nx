...
Nx evolved from being an extension of the Angular CLI to a [fully standalone CLI working with multiple frameworks](/docs/getting-started/intro#start-small-extend-as-you-grow). As a result, adopting Nx as an Angular user is relatively straightforward. This guide explores some of the similarities and in particular, added benefits of using Nx over the Angular CLI for your Angular project.
...
| Task Graph                                                                                                 | ❌              | ✅            |
| [Running Tasks in Parallel](/docs/guides/tasks--caching/run-tasks-in-parallel)                             | ❌              | ✅            |
| Building, Testing [Only What is Affected](/docs/features/run-tasks#run-tasks-on-projects-affected-by-a-pr) | ❌              | ✅            |
...
More details: Nx [project configuration](/docs/reference/project-configuration).
...
**Angular Builders** are called [Executors](/docs/concepts/executors-and-configurations) in Nx but work very much similarly. You use them in your project configuration to define how to build, test, lint, and serve your project. You can use both Nx executors from [Nx Plugins](/docs/plugin-registry) or Angular Builders from the Angular Devkit.
...
**Angular Schematics** are called [Generators](/docs/features/generate-code) in Nx. You can invoke them in the same way as you would with the Angular CLI, but you use the `nx` command instead of `ng`:
...
Support to run Angular Devkit builders and schematics is enabled by installing the [`@nx/angular` plugin](/docs/technologies/angular/introduction). This plugin is installed by default when creating a new Angular workspace with Nx or [migrate an existing Angular CLI workspace to Nx](#migrate-from-the-angular-cli).
{% /aside %}
...
Nx has more abilities to run commands in parallel, just for specific projects etc. Find out more [in the docs](/docs/features/run-tasks).
...
To reiterate: `nx migrate` runs the migrations written by the Angular team the same way `ng update` runs them. So everything should still work. You just get more control over how it works. You can learn more in our docs about [Automate Updating Dependencies](/docs/features/automate-updating-dependencies).

### 'nx add'

The [`nx add` command](/docs/reference/nx-commands#nx-add) is similar to the `ng add` command. It installs a given package specifier (e.g. `@nx/react`, `@nx/react@18.1.0`, `@nx/react@latest`) and it runs an `init` or `ng-add` generator if the installed package contains it.
...
Nx is designed to be fast. The Angular CLI leverages Webpack caching, which Nx also does since it relies on the Angular Devkit when it comes to compiling or running apps. But Nx goes way beyond that, introducing features vital for a fast CI experience, mainly when you have a monorepo.
...
Nx goes beyond being just a CLI and comes with [Nx Console](/docs/getting-started/editor-setup), a VSCode and WebStorm extension allowing you to run commands, generate code and visualize your workspace.
...
- modularize your application into more fine-grained libraries for better maintainability as your application (and team) grows, including mechanisms to make sure [things stay within their boundaries](/docs/features/enforce-module-boundaries)
- you can then migrate to a monorepo when you are ready and need one ([more here](/docs/guides/tips-n-tricks/standalone-to-monorepo))
- or even [add Webpack Module Federation support](/docs/technologies/angular/guides/module-federation-with-ssr)

### Visualize your workspace

As you start modularizing your Angular workspace, Nx can visualize it using the `nx graph` command or via [Nx Console](/docs/getting-started/editor-setup) directly in your editor.
...
Learn more about the [graph features here](/docs/features/explore-graph).
