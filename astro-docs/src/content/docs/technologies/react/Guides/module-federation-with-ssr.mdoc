...
sidebar:
  label: 'Setup Module Federation with SSR'
filter: 'type:Guides'
---

Walk you through creating a Module Federated setup with Server Side Rendering (SSR) for Angular and React using Nx and its generators.
...
```shell frame="terminal" title="npx create-nx-workspace@latest myorg --preset=apps"
// filename: ~
...
{% card title="Opting into Nx Cloud" description="You will also be prompted whether to add Nx Cloud to your workspace. We won't address this in this recipe, but you can see the introduction to Nx Cloud for more details." url="/docs/guides/nx-cloud/setup-ci" /%}
...
```shell frame="terminal" title="nx add @nx/angular"
// filename: ~/myorg
...
```shell frame="terminal" title="nx add @nx/react"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx g @nx/angular:host apps/store --ssr --remotes=product,checkout"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx g @nx/react:host apps/store --ssr --remotes=product,checkout"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx serve-ssr store"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx serve store"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx serve-ssr store --devRemotes=checkout"
// filename: ~/myorg
...
```shell frame="terminal" title="npx nx serve store --devRemotes=checkout"
// filename: ~/myorg
