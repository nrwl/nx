...
![Host with implicit dependencies to remotes](../../../../../assets/concepts/module-federation/dep-graph-2.png)

## Creating an example workspace

The best way to understand the setup is through an example. we will create a `host` application with
...
The `shared` function can return an `undefined` to use Nx default value, `false` to exclude it from being shared, or
...
![Nx Cloud run details](../../../../../assets/concepts/module-federation/nx-cloud.png)

## Production build and deployment with Nx Cloud

We'll examine how to set up your production build and simulate a deployment to `http://localhost:3000`.
