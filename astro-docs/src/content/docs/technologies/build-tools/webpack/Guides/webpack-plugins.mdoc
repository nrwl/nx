...
Takes a library name and the current [share configuration](https://webpack.js.org/plugins/module-federation-plugin/#sharing-hints), and returns one
of
...
{% tabs %}
{% tabitem label="React Module Federation" %}
...
{% /tabitem %}
{% tabitem label="React Module Federation for SSR " %}
