...
jobs:
  - job: main
    ...
    steps:
      ...
      - run: npx nx start-ci-run --distribute-on="8 linux-medium-js" --stop-agents-after="e2e-ci"
      - ...
```
...
{% aside type="caution" title="The order of your changesets matters!" %}
...
Nx Cloud calculates the relationship between the number of [affected projects](/docs/features/ci-features/affected) and the total number of projects in the workspace to determine the size of a PR.
{% /callout %}
...
jobs:
  - job: main
    ...
    steps:
      ...
      - run: npx nx start-ci-run --distribute-on=".nx/workflows/distribution-config.yaml" --stop-agents-after="e2e-ci"
      - ...
```
