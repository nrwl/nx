...
And the following [final configuration](/docs/reference/project-configuration):
...
{% tabs %}
{% tabitem label="Setting args directly as options" %}

{% aside type="note" title="Providing command args as options" %}
...
{% tabs %}
{% tabitem label="Providing args directly as options" %}
...
{% tabitem label="Providing the \"--args\" option" %}
...
{% aside type="caution" title="Conflicting options" %}
