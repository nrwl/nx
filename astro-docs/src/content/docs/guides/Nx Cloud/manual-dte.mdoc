  ...
  main:
    ...
    steps:
      ...
      - name: Initialize the Nx Cloud distributed CI run and stop agents when the build tasks are done
        run: npx nx start-ci-run --distribute-on="manual" --stop-agents-after=e2e-ci

      - name: Check the formatting
        run: npx nx record -- nx format:check
...
jobs:
  ...
  - job: main
    ...
    steps:
      ...
      - script: npm ci
      - script: npx nx start-ci-run --distribute-on="manual" --stop-agents-after="e2e-ci"
      - script: npx nx record -- nx format:check --base=$(BASE_SHA) --head=$(HEAD_SHA)
      ...
      - parallel:
          - step:
              ...
              script:
                ...
                - npm ci
                - npx nx start-ci-run --distribute-on="manual" --stop-agents-after="e2e-ci"
                - npx nx record -- nx format:check
...
nx-dte:
  ...
  script:
    - yarn nx start-ci-run --distribute-on="manual" --stop-agents-after=e2e-ci
    - yarn nx record -- nx format:check --base=$NX_BASE --head=$NX_HEAD
        ...
        stage('Pipeline') {
            parallel {
                stage('Main') {
                    ...
                    steps {
                        sh "npm ci"
                        sh "npx nx start-ci-run --distribute-on='manual' --stop-agents-after='e2e-ci'"
                        sh "npx nx record -- nx format:check"
