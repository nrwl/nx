...
jobs:
  main:
    ...
    steps:
      ...
      - run: npx nx start-ci-run --distribute-on="5 linux-large" --stop-agents-after="e2e"
      ...
      - run: bunx nx record -- nx format:check
...
The `nx start-ci-run` command runs before dependencies are installed,
...
```shell
npx nx start-ci-run --distribute-on="5 linux-large" --stop-agents-after="e2e"
```
