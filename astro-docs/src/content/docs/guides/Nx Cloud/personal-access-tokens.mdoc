...
From Nx 19.7 repositories are connected to Nx Cloud via a property in `nx.json` called `nxCloudId`. By default this value allows anyone who clones the repository `read-write` access to Nx Cloud features for that workspace. These permissions can be updated in the workspace settings. To disallow access to anonymous users or allow `read-write` access to known users it is required that all users provision their own personal access token. To do that they need to use [`npx nx login`](/docs/reference/nx-cloud-cli#npx-nxcloud-login).

{% aside type="caution" title="Personal Access Tokens require the `nxCloudId` field in `nx.json`" %}
...
![Profile Settings](../../../../assets/nx-cloud/profile-page.avif)

From there, click on the `Personal access tokens` tab.

![Personal Access Tokens](../../../../assets/nx-cloud/personal-access-tokens-profile.avif)
...
You can then use [nx-cloud configure](/docs/reference/nx-cloud-cli#npx-nxcloud-configure) in your terminal to set the token in your local configuration file.
...
These are the permissions granted to users who are not [logged in](/docs/reference/nx-cloud-cli#npx-nxcloud-login) or are not members of the Nx Cloud organization for this workspace. By default, all users have `read-write` access to the workspace. This can be updated in the workspace settings to `read-only` or `none`.
...
When a workspace member logs in with a personal access token after running [`npx nx login`](/docs/reference/nx-cloud-cli#npx-nxcloud-login) they are granted access to Nx Cloud features.
