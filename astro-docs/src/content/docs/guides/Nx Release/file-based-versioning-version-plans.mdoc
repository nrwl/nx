...
Both in this recipe, but first we need to enable the feature itself.
...
```md
## // .nx/version-plans/version-plan-1723732065047.md
...
{% aside type="note" title="Single Version for All Packages" %}

If you use a single version for all your packages (see [Release projects independetly](/docs/guides/nx-release/release-projects-independently)) your version plan file might look like this:
...
To make things easier, Nx release comes with a built in command to help you generate valid version plan files. See the [`nx release plan`](/docs/reference/nx-commands#nx-release-plan) CLI reference for all available options.
