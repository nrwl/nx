...
description: >-
  ...
  block, and the AI-agent prompt/runbook .md for prompt-only or hybrid
...
| Ported upstream migration | Upstream ships its own migrations for the major                                                         | One generator-only migration per upstream migration, description ending "matching the <upstream> `X` migration", `requires` on the new major (see `packages/angular/migrations.json` `update-23-1-0-add-trust-proxy-headers`)                                   |
...
- Every new entry gets a colocated `documentation` .md per [templates/documentation-md.md](templates/documentation-md.md): before/after samples for generator-based migrations, what-the-upgrade-involves for prompt migrations (`upgrade-to-<framework>-<major>.md`); h4/h5 headings only. The key is read only by the agentic flow; the docs site instead inlines the .md sharing the implementation's basename, so for implementation-carrying entries one file named after the implementation serves both. A prompt .md never doubles as documentation: it is agent-voiced, wrong audience (see `packages/react/migrations.json` `update-23-1-0-create-ai-instructions-for-react-19`, which pairs both).
