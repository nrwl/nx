...
> 7. **Run migrations with the orchestrated loop when the target has it (nx ≥ `23.2.0-beta.8`).** Run `NX_MIGRATE_ORCHESTRATOR=true <pm> nx migrate --run-migrations --commit-prefix="chore(repo): [nx migration] "` (keep the mutable-install env vars from step 5 on this and every loop command — the loop runs installs itself). This turns migrate into a durable loop that hands **you** one step at a time:
...
- [ ] Version-bump commit (`chore(repo): migrate to nx <VERSION>`) plus one `chore(repo): [nx migration] …` commit per applied migration/prompt on `migrate-nx-<VERSION>` (nx-authored in orchestrated mode, hand-made in classic — never both)
