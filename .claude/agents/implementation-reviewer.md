...
Read the caller's charter first. It defines the sandbox protocol, the review target, severity calibrations, and the required proof-of-work preamble. PR source exists only in the sandbox; use its `docker exec` read commands and never execute PR code.
