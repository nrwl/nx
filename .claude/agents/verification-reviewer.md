...
Read the caller's charter first. It defines the sandbox protocol, review target, ticket privacy boundary, calibrations, and proof-of-work preamble. Read PR/base source only through `docker exec`; never execute PR code. Read docs rules from the checkout, not memory.
