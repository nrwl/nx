...
7. **Compare against the base when unsure.** If it's unclear whether a cost is new, read the same code on the base from the host clone (`git -C "$NX_REPO_PATH" show origin/<BASE_REF>:<path>`). Pre-existing cost the PR merely relocates is not a finding.
