...
2. **Hunt injection sinks.** In changed code, look for:
   ...
   - **Non-obvious execution primitives — quoting does NOT neutralize these.** A value being inside double quotes, or passed as a single argument, is not safety by itself:
     - **GNU `sed`** runs shell commands via its `e` command (and reads/writes files via `r`/`w`), so `sed -n "${UNTRUSTED}p"` or `sed "$UNTRUSTED"` with an attacker-controlled address/script is code execution — the value must be validated (e.g. pure-digit) _before_ it reaches `sed`, not merely quoted.
     - **Shell assignment-prefix:** `VAR=<untrusted> cmd` parses as "set VAR for the duration of `cmd`" and **runs `cmd`** — so a bare `LINE=<paste-of-attacker-text>` is host RCE _before any later gate runs_. The assignment is itself a sink. A value is only safe from this if it never reaches a bare assignment: round-trip it through a file (`Write` + `VAR=$(cat file)`, which never re-parses the bytes) or strictly pre-validate it.
     - **Others:** `awk` `system()`/`getline`, `printf %b` / `printf -v`, arithmetic `$(( <untrusted> ))`, `[ ]`/`test` with unquoted operands, `find -exec`, `xargs`, and glob/word-splitting on any unquoted expansion of an untrusted value.
